...
import {
    ...
    FaUser, FaPlus, FaUpload, FaCreditCard, FaUserMd, FaStethoscope
} from "react-icons/fa";
...
export default function CheckoutPage() {
    ...
    const handleMemberSelect = (member) => {
        if (member === "self" || member === "add") {
            ...
            const fullAddress = [member.houseNo, member.sector].filter(Boolean).join(", ");
            setPatientDetails({
                ...patientDetails,
    ...
    const handleApplyCoupon = async (codeToApply = couponCode) => {
        ...
        try {
            ...
            if (response.success) {
                setAppliedCoupon({ ...response.data, couponName: codeToApply });
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                            ...
                            <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide">
                                ...
                                {familyMembers.map((member) => (
                                    <div key={member._id} onClick={() => handleMemberSelect(member)} className="flex-shrink-0 cursor-pointer flex flex-col items-center gap-2 group">
                                        <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-all bg-slate-100 ${selectedMemberId === member._id ? "border-emerald-500 ring-4 ring-emerald-50" : "border-transparent"}`}>
                                            <div className={`w-full h-full rounded-full flex items-center justify-center text-sm font-black ${selectedMemberId === member._id ? "bg-emerald-100 text-emerald-600" : "bg-slate-200 text-slate-500"}`}>
                                                {member.name ? member.name.charAt(0).toUpperCase() : <FaUser />}
                                            </div>
                                        </div>
                                        <span className={`text-[10px] font-black uppercase tracking-tight ${selectedMemberId === member._id ? "text-emerald-600" : "text-slate-400"}`}>{member.name}</span>
                                    </div>
                                ))}
                            ...
                            <div className="md:col-span-2 pt-8">
                                ...
                                {patientDetails.haveInsurance === "Yes" && (
                                    <div className="bg-[#FFF9F4] p-8 rounded-[2rem] border border-[#FFE7D6] space-y-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <input type="text" name="insuranceNo" placeholder="Insurance No." className="bg-white px-4 py-3 rounded-xl outline-none font-bold text-slate-700" onChange={handleInputChange} />
                                            <input type="text" name="companyName" placeholder="Company Name" className="bg-white px-4 py-3 rounded-xl outline-none font-bold text-slate-700" onChange={handleInputChange} />
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                {selectedDoctorId && (
                                    <button onClick={() => setSelectedDoctorId(null)} className="text-[9px] font-black text-red-500 bg-red-50 px-3 py-1.5 rounded-full uppercase hover:bg-red-100 transition-colors">Clear Selection</button>
                                )}
                            </div>
                                        ...
                                        <div key={service._id} onClick={() => toggleService(service._id)} className={`cursor-pointer p-4 rounded-2xl border-2 transition-all flex items-center justify-between ${isSelected ? 'border-emerald-500 bg-emerald-50/30' : 'border-slate-50 hover:border-slate-200 bg-white'}`}>
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-slate-400 font-medium tracking-tight line-clamp-1">{service.description}</p>
                                                </div>
                                            </div>
