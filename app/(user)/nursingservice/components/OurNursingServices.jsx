...
function OurNursingServices() {
  ...
  const pageData = {
    title: "Our Services",
    description: "Compassionate, professional nursing care at your doorstep. Our team ensures safety and personalized medical plans for faster recovery.",
    brandColor: "#08B36A",
    services: [
      "Personal Caring",
      "Nursing In Our Home",
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentImg((prev) => (prev === pageData.carouselImages.length - 1 ? 0 : prev + 1));
    }, 2500);
  ...
  return (
    <section className="py-8 md:py-16 bg-[#f8fafc] font-sans overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Mobile: Stacked | Tablet & Desktop: Side-by-Side (Grid) */}
        <div className="grid grid-cols-1 md:grid-cols-2 items-stretch gap-4 md:gap-0">
          
          {/* LEFT: IMAGE CAROUSEL (order-1 always to be on top in mobile) */}
          <div className="relative h-[220px] sm:h-[300px] md:h-auto min-h-[300px] md:min-h-[450px] overflow-hidden rounded-t-[2rem] md:rounded-t-none md:rounded-l-[2.5rem] shadow-xl md:shadow-none z-10">
            {pageData.carouselImages.map((img, index) => (
              <div
                key={index}
            ...
            <div className="absolute bottom-4 right-6 z-20 flex gap-1.5">
               {pageData.carouselImages.map((_, i) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 sm:gap-y-4 gap-x-4 pt-4">
                {pageData.services.map((service, index) => (
                  <div key={index} className="flex items-center gap-2 group/item cursor-default">
                    ...
                    <span className="font-bold text-[10px] md:text-xs lg:text-sm uppercase tracking-wider group-hover/item:pl-1 transition-all">
                      {service}
                    </span>
                  </div>
                ))}
