...
import NurseDetailsModal from "../components/otherComponents/NurseDetailsModel";

const CATEGORIES = ["All", "End of Life Care", "ICU Specialist", "General Nursing"];
...
export default function AllNursesPage() {
  ...
  const filteredNurses = useMemo(() => {
    let result = NURSES_DATA.filter((nur) => {
      const matchesSearch =
        nur.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        nur.speciality.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesCategory = activeCategory === "All" || nur.speciality === activeCategory;
  ...
  return (
    <div className="min-h-screen font-sans selection:bg-[#08B36A]/10 bg-white">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 py-7">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 text-slate-400">
            <FaUserNurse className="text-[#08B36A] text-sm" />
            <span className="text-[9px] font-black uppercase tracking-tighter">Verified Home Care</span>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto py-6 md:py-10 px-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tight">
              Expert <span className="text-[#08B36A]">Nursing Staff</span>
            </h1>
            <p className="text-slate-500 text-[10px] md:text-sm font-medium">Professional healthcare at your doorstep.</p>
          </div>
          ...
          <div className="flex gap-2 w-full md:w-auto">
            ...
            <div className="relative">
              ...
              >
                <option value="low-to-high">Rate: Low</option>
                <option value="high-to-low">Rate: High</option>
              </select>
        ...
        {/* GRID: 2 COL MOBILE, 3 COL LAPTOP */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
          {filteredNurses.map((nur) => (
            <div
              key={nur.id}
              onClick={() => handleNurseClick(nur)}
            ...
            >
              <div className="h-28 sm:h-36 md:h-48 w-full relative overflow-hidden rounded-xl bg-slate-50 mb-3">
                ...
                <div className="absolute top-2 left-2">
                  <span className={`px-2 py-0.5 rounded text-[7px] font-black uppercase tracking-widest text-white ${nur.available ? 'bg-[#08B36A]' : 'bg-slate-400'}`}>
                    {nur.available ? 'Available' : 'Booked'}
                  </span>
                </div>
              ...
              <div className="mt-4 flex items-center justify-between pt-2 border-t border-slate-50">
                <div>
                  <p className="text-[7px] font-bold text-slate-400 uppercase">Per Day</p>
                  <p className="text-sm md:text-xl font-black text-slate-900">₹{Math.floor(nur.price)}</p>
                </div>
                <button
        ...
        {filteredNurses.length === 0 && (
          <div className="text-center py-20 border-2 border-dashed border-slate-100 rounded-3xl mt-10">
            <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No nursing staff found</p>
            <button
