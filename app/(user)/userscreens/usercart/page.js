...
const CartPage = () => {
    ...
    // Local state to track if we've done the very first load
    ...
    // ONLY show full screen loader on the very first time the page opens
    ...
    return (
        <div className="min-h-screen bg-[#F3F7F5] pb-32 md:pb-10">
            ...
            <div className="max-w-6xl mx-auto p-4 md:p-10">
                ...
                {cartItems.length === 0 && initialLoadDone ? (
                                        ...
                                        <div className="flex items-end justify-between mt-2">
                                            ...
                                            <div className="flex items-center bg-slate-50 rounded-lg p-1 border border-slate-100 scale-90 md:scale-100 origin-right">
                                                <button
                                                    onClick={(e) => {
                                                        e.preventDefault();
                                                        if (item.quantity > 1) updateQuantity(item.itemId._id, 'dec');
                                                    }}
                                                    disabled={loading}
                                                    className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-[#08b36a] disabled:opacity-50"
                                                >
                                                    <FaMinus size={12} />
                                                </button>
                                                ...
                                                <button
                                                    onClick={(e) => {
                                                        e.preventDefault();
                                                        updateQuantity(item.itemId._id, 'inc');
                                                    }}
                                                    disabled={loading}
                                                    className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-[#08b36a] disabled:opacity-50"
                                                >
