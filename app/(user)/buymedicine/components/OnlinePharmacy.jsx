...
import {
  ...
  FaPlus, FaHistory, FaBolt, FaStethoscope, FaTimes
} from "react-icons/fa";
...
function OnlinePharmacy() {
  ...
  // Initial Content and Inventory Load
  useEffect(() => {
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 relative" ref={searchRef}>
              <div className="relative flex-1 group">
                ...
                {showSuggestions && (
                  <div className="absolute top-[115%] left-0 w-full bg-white rounded-2xl shadow-[0_25px_60px_-15px_rgba(15,23,42,0.12)] border border-slate-100 overflow-hidden z-[100] animate-in fade-in zoom-in-95 duration-200">
        ...
        <section>
          <div className="flex items-center justify-between mb-8 px-1">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400 font-medium">Currently showing live products in our inventory</p>
            </div>
          ...
          {medsLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-6">
              {filteredMedicines.slice(0, 8).map((med, index) => {
                ...
                // Parse discount metrics securely
                const discountPercentStr = med.discont_percent || med.discount || "";
                const discountPercentVal = parseInt(discountPercentStr.toString().replace('%', '')) || 0;

                // Out of stock evaluation
                const isOutOfStock = med.isAvailable === false || med.isAvailable === "false";

                return (
                  <div
                    key={med._id || med.Id || index}
                    onClick={() => router.push(`/buymedicine/singleproductdetail/${med._id || med.Id}`)}
                    className={`group cursor-pointer flex flex-col bg-white border border-slate-100 rounded-3xl overflow-hidden hover:border-transparent hover:shadow-[0_22px_50px_rgba(148,163,184,0.12)] transition-all duration-500 relative ${
                  ...
                  >
                    ...
                    <div className="relative aspect-square bg-slate-50/60 overflow-hidden flex items-center justify-center">
                      <img
                        ...
                        className={`h-full w-full object-cover transition-transform duration-700 ease-out ${
                          isOutOfStock ? "grayscale opacity-40 scale-95" : "group-hover:scale-105"
                        }`}
                      />

                      {/* Out of Stock and Discount Overlays */}
                      {isOutOfStock ? (
                        <div className="absolute top-4 right-4 bg-rose-50 text-rose-600 border border-rose-100 text-[10px] font-extrabold px-2.5 py-1 rounded-full shadow-sm tracking-wide uppercase">
                          Out of Stock
                        </div>
                      ) : (
                    ...
                    <div className="p-5 flex flex-col flex-1 bg-white">
                      ...
                      <h3 className={`text-[15px] font-bold line-clamp-2 h-11 leading-snug transition-colors duration-300 ${
                        isOutOfStock ? "text-slate-400" : "text-slate-800 group-hover:text-emerald-700"
                      }`}>
                        {med.name}
                      </h3>

                      {med.salt_composition && (
                        <p className="text-[10px] text-slate-400 font-semibold truncate mt-1">{med.salt_composition}</p>
                      )}
                      ...
                      <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                        <div>
                          ...
                          <span className={`text-lg font-extrabold tracking-tight ${isOutOfStock ? "text-slate-400" : "text-slate-900"}`}>
                            ₹{displayPrice}
                          </span>
                        </div>
                        
                        <div 
                          className={`px-4.5 h-10 rounded-xl flex items-center justify-center text-xs font-bold transition-all duration-300 shadow-md shrink-0 select-none ${
                            isOutOfStock 
                              ? "bg-slate-100 text-slate-400 cursor-not-allowed shadow-none" 
                              : "bg-slate-900 text-white hover:bg-emerald-600 hover:shadow-emerald-100 active:scale-95"
                          }`}
                        >
            ...
            <div className="w-full text-center py-20 bg-white rounded-3xl border border-dashed border-slate-200 shadow-sm">
              ...
              <p className="text-slate-400 text-xs mt-1">No operational products found inside the network cluster matching the search parameters.</p>
            </div>
          )}
        </section>
