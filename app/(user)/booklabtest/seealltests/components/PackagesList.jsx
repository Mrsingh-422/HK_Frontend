    ...
    useEffect(() => {
        const fetchPackages = async () => {
            try {
                ...
                if (selectedLabId) {
                    // Fetching packages for a SPECIFIC lab
                    response = await UserAPI.getLabDetails(selectedLabId);
                    if (response.success) {
                        setPackages(response.packages || []);
                    }
                } else {
                    // Fetching the GENERAL catalog
                    response = await UserAPI.getStandardPackageCatalog();
                    if (response.success) {
                        // Handle both response.data or response.packages patterns
        ...
        <div className="bg-transparent">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
                {filteredPackages.map((pkg) => {
                    ...
                    // Use offerPrice for lab specific, minPrice for general catalog
                        ...
                        >
                            ...
                            <div className="p-3 md:p-5 flex-1 flex flex-col">
                                ...
                                <div className="mt-2 md:mt-3 flex items-center gap-2 text-[9px] md:text-[11px] text-slate-400 font-bold uppercase tracking-tighter">
                                    ...
                                    <span>{pkg.testCount || pkg.tests?.length || 'Comprehensive'} Tests Included</span>
                                </div>
