...
export default function PackageDetailPage() {
    ...
    if (pageLoading) return (
        <div className="h-screen flex flex-col items-center justify-center bg-white">
            ...
            <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">Loading Package...</p>
        </div>
    );

    if (!pkg) return <div className="h-screen flex items-center justify-center font-bold">Package not found</div>;
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <div className="flex-1">
                        <div className="mb-8">
                            <div className="flex flex-wrap gap-2 mb-4">
                                <span className="bg-emerald-100 text-emerald-700 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">{pkg.mainCategory}</span>
                                <span className="bg-slate-100 text-slate-600 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">{pkg.category}</span>
                            </div>
                            <h1 className="text-5xl font-black text-slate-900 uppercase leading-[0.9] mb-4">{pkg.packageName}</h1>
                            <p className="text-slate-500 text-lg font-medium leading-relaxed max-w-2xl">{pkg.shortDescription}</p>
                        </div>
                                    ...
                                    {labs.map((lab) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-5">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center gap-4 mt-1">
                                                        <span className="flex items-center gap-1 text-xs font-black text-amber-500"><FaStar /> {lab.rating}</span>
                                                        <span className="text-[10px] text-slate-400 font-bold uppercase">{lab.distance} km from you</span>
                                                        {lab.isHomeCollection && <span className="text-[10px] text-emerald-600 font-bold uppercase">Home Visit</span>}
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="md:text-right w-full md:w-auto pt-4 md:pt-0 border-t md:border-none border-slate-100">
                                                ...
                                                {lab.mrp > lab.offerPrice && <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest">Special Offer</p>}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <section className="mb-12">
                            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 mb-6 flex items-center gap-2">
                                <div className="w-8 h-[2px] bg-blue-500"></div> Tests & Parameters
                            </h3>
                            <div className="grid md:grid-cols-2 gap-4">
                                {pkg.tests?.map((test) => (
                                    <div key={test._id} className="p-6 border border-slate-100 rounded-[2rem] bg-white shadow-sm">
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-500"><FaMicroscope size={14} /></div>
                                            <h4 className="font-black uppercase text-slate-800 text-xs tracking-wider">{test.testName}</h4>
                                        </div>
                                        <div className="flex flex-wrap gap-2">
                                            {test.parameters?.map((p, i) => (
                                                <span key={i} className="text-[9px] bg-slate-50 border border-slate-100 px-3 py-1 rounded-full uppercase font-bold text-slate-400 tracking-tighter">{p}</span>
                                            ))}
                                        </div>
                                    </div>
                                ))}
                        ...
                        {pkg.faqs?.length > 0 && (
                            <section className="mb-12">
                                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 mb-6">Common Questions</h3>
                    ...
                    <div className="w-full lg:w-96">
                        <div className="sticky top-24 bg-white border border-slate-100 rounded-[3rem] p-10 shadow-2xl shadow-slate-200/50">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Total Package Price</p>

                            <div className="flex items-baseline gap-3 mb-8">
                                <span className="text-6xl font-black text-slate-900 tracking-tighter">₹{selectedLab?.offerPrice || pkg.standardMRP}</span>
                                {selectedLab?.mrp > selectedLab?.offerPrice && (
                                    <span className="text-slate-300 line-through font-bold text-xl">₹{selectedLab.mrp}</span>
                                )}
                            </div>
                            ...
                            <div className="space-y-4 mb-10">
                                <div className="flex items-center gap-3 p-4 bg-emerald-50 rounded-2xl">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-emerald-700 uppercase tracking-wider">Verified Lab</p>
                                        <p className="text-[10px] font-bold text-emerald-600/70 uppercase">Sample collection included</p>
                                    </div>
                                </div>
                                <div className="p-4 border border-slate-100 rounded-2xl">
                                    <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Preparation</p>
                                    <p className="text-[11px] font-bold text-slate-600 uppercase">{pkg.preparations?.join(", ") || "No special instructions"}</p>
                                </div>
                            </div>
