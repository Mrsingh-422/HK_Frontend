...
export default function BookingConfirmation() {
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            fetchSelectionData(); // Fetch Addresses and Family
            fetchVisitCharges(parsed.doctorId); // Fetch Visit Charges
        } else {
            router.push('/');
        }
    }, [router]);
    ...
    // Pricing Logic (Updated with Visit Charges)
    const pricing = useMemo(() => {
        ...
        // Logic for Home Care Service
        if (bookingData?.selectedService === 'Home Care' && visitCharges) {
            homeVisitFee = Number(visitCharges.fixedPrice || 0);
            const distance = Number(bookingData?.distance || 0); // Assuming distance is passed in bookingData

            if (distance > visitCharges.fixedDistance) {
                const extraKm = distance - visitCharges.fixedDistance;
        ...
        <div className="min-h-screen bg-white text-slate-800 pb-20 font-sans">
            ...
            <main className="max-w-6xl mx-auto px-6 pt-10">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-5">
                        <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 sticky top-24">
                            ...
                            <div className="space-y-3 mb-8">
                                ...
                                {pricing.travelFee > 0 && (
                                    <div className="flex justify-between text-sm">
                                        <span className="text-slate-500">Travel Charge ({bookingData.distance} km)</span>
                                        <span className="font-bold">+₹{pricing.travelFee}</span>
                                    </div>
                                )}
                                <div className="flex justify-between text-sm">
                      ...
                      ${appliedCoupon ? 'bg-rose-50 text-rose-600' : 'bg-slate-900 text-white disabled:bg-slate-200'}`}
                            ...
                            <button
                                disabled={!selectedSlot || !selectedMember || !selectedAddress}
                  ...
                  ${(selectedSlot && selectedMember && selectedAddress) ? 'bg-emerald-600 text-white shadow-lg hover:bg-emerald-700' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
                                onClick={() => console.log("Finalizing Booking for:", selectedMember.memberName)}
                            >
