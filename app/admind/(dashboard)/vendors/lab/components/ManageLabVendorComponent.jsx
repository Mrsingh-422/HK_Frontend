...
function ManageLabVendorComponent() {
  ...
  useEffect(() => {
    const initPage = async () => {
      setIsLoading(true);
      try {
        const [labRes, countryData] = await Promise.all([
          AdminAPI.getLabsList(),
          getAllCountries()
        ]);
        setVendors(labRes.data || []);
  ...
  if (isLoading) return (
    <div className="w-full h-screen flex flex-col items-center justify-center bg-white">
      <div className="relative flex items-center justify-center">
        ...
        <FaFlask className="absolute text-emerald-500" size={24} />
      </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-3xl font-black text-slate-900 flex items-center gap-3 tracking-tight">
              Lab Audit
              <span className="bg-emerald-500 text-white text-[10px] px-2 py-1 rounded-md uppercase">Vendor</span>
            </h1>
          ...
          <div className="overflow-x-auto">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-400 font-bold mt-1 tracking-tight flex items-center gap-1">
                            <FaFlask className="text-emerald-500/50" size={10} /> {item.phone}
                          </p>
                        </div>
                      </div>
                    </td>
            ...
            {filteredData.length === 0 && (
              <div className="py-24 text-center bg-slate-50/20">
                ...
                <p className="text-slate-400 font-black uppercase tracking-widest text-[10px]">No vendor matches found</p>
              </div>
            )}
          </div>
        </div>
