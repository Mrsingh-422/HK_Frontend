...
import { 
  FaSearch, FaPlus, FaRegTrashAlt, FaPills, 
  FaArrowLeft, FaCloudUploadAlt, FaTimes, 
  FaCheckCircle, FaLink, FaEye, FaRegEdit,
  FaMedkit
} from 'react-icons/fa';

export default function AppMedicineBanners() {
  // --- STATE MANAGEMENT ---
  const [banners, setBanners] = useState([
    { id: 1, title: "Flat 25% Off on Vitamins", image: "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?q=80&w=500&auto=format&fit=crop", link: "/store/vitamins", status: "Active", date: "2024-03-11" },
    { id: 2, title: "Essential Healthcare Kit", image: "https://images.unsplash.com/photo-1583947215259-38e31be8751f?q=80&w=500&auto=format&fit=crop", link: "/store/essentials", status: "Active", date: "2024-03-14" },
    { id: 3, title: "Baby Care Essentials Sale", image: "https://images.unsplash.com/photo-1515488764276-beab7607c1e6?q=80&w=500&auto=format&fit=crop", link: "/store/babycare", status: "Inactive", date: "2024-03-02" },
  ]);
  ...
  const handleSubmit = (e) => {
    ...
    if (modalMode === "add") {
      const newEntry = {
        ...
        image: formData.image || "https://via.placeholder.com/500x200?text=Medicine+Banner"
      };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Delete this pharmacy promotional banner?")) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-800 tracking-tight leading-none uppercase">Medicine App Banners</h1>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 text-center md:text-left">Promote Pharmacy Inventory & Offers</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-50 bg-white flex justify-between">
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search medicine campaigns..." 
            ...
            <table className="w-full text-left border-separate border-spacing-y-3">
              <thead>
                <tr className="text-slate-400 text-[10px] font-black uppercase tracking-widest">
                  <th className="px-6 py-2">Graphics</th>
                  <th className="px-6 py-2">Promotion Details</th>
                ...
                {filteredBanners.map((banner) => (
                  <tr key={banner.id} className="group transition-all bg-slate-50/50 rounded-2xl">
                    ...
                    <td className="px-6 py-4">
                        <p className="text-sm font-black text-slate-700 leading-tight">{banner.title}</p>
                        <p className="text-[10px] font-bold text-emerald-600 mt-1 uppercase tracking-tighter flex items-center gap-1">
                            <FaLink size={8}/> {banner.link}
                        </p>
                    </td>
            ...
            <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-[#F8FAFC]">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-black uppercase text-slate-800 tracking-tight">
                        {modalMode === "add" ? "New Pharmacy Offer" : "Edit Campaign"}
                    </h3>
                </div>
            ...
            <form onSubmit={handleSubmit} className="p-10 space-y-6">
                <div className="w-full h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center text-slate-300 group hover:border-[#08B36A] transition-all overflow-hidden relative shadow-inner">
                    {formData.image ? (
                        ...
                        <>
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest mt-2">Upload Pharmacy Graphics</p>
                        </>
                    )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input value={formData.title} onChange={(e)=>setFormData({...formData, title: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-[#08B36A]/20 transition-all" placeholder="e.g. Antibiotics at 10% Discount" required />
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Store URL Path</label>
                        <div className="relative">
                            <FaLink className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={12}/>
                            <input value={formData.link} onChange={(e)=>setFormData({...formData, link: e.target.value})} className="w-full pl-10 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-700 outline-none" placeholder="/pharmacy/offers" required />
                        </div>
                    </div>
                    <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Source Link (Demo)</label>
                    <input value={formData.image} onChange={(e)=>setFormData({...formData, image: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm text-slate-500 outline-none" placeholder="Paste image URL..." />
                  </div>
                </div>
                ...
                <button type="submit" className="w-full py-5 bg-[#08B36A] text-white rounded-3xl font-black uppercase text-xs tracking-[0.2em] shadow-xl shadow-green-100 flex items-center justify-center gap-2 active:scale-95 transition-all">
                  <FaCheckCircle /> {modalMode === "add" ? "Deploy Banner" : "Update Live Data"}
                </button>
            </form>
          </div>
        </div>
