...
export default function NurseOrdersPage() {
  ...
  const [orders, setOrders] = useState([
    { srNo: 1, id: "NRS-5501", patient: "Yash User", email: "HK@Dev123", agency: "LifeCare Nursing", time: "10:00 AM", status: "Accepted", date: "2024-03-20", service: "Wound Dressing" },
    { srNo: 2, id: "NRS-5502", patient: "Nitish Kumar", email: "nitish@hk.com", agency: "HealthFirst Agency", time: "11:30 AM", status: "Pending", date: "2024-03-21", service: "IV Infusion" },
    { srNo: 3, id: "NRS-5503", patient: "NAMAN SHARMA", email: "naman@gmail.com", agency: "Angel Home Care", time: "02:00 PM", status: "Accepted", date: "2024-03-20", service: "Post-Op Monitoring" },
    { srNo: 4, id: "NRS-5504", patient: "Deepankar", email: "deep@gmail.com", agency: "City Support Staff", time: "04:00 PM", status: "Cancelled", date: "2024-03-22", service: "Vital Checkup" },
  ]);
  ...
  // Search filter (ID, Patient, or Agency)
  const filteredOrders = useMemo(() => {
    return orders.filter(order => 
      order.patient.toLowerCase().includes(searchTerm.toLowerCase()) || 
      order.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      order.agency.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, orders]);
  ...
  const handleCancelOrder = (e, id) => {
    e.stopPropagation(); 
    if (window.confirm("Are you sure you want to cancel this nursing home-visit?")) {
      setOrders(orders.filter(order => order.id !== id));
      alert("Nurse Order Cancelled Successfully.");
    }
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 text-center md:text-left">Home Care & Nursing Registry</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="p-6 border-b border-slate-50 bg-white">
            <div className="relative max-w-md group">
              ...
              <input 
                type="text" 
                placeholder="Search patient, ID or agency..." 
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
                  ...
                  <th className="p-6">Vendor (Nursing Agency)</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white/20">
            ...
            <div className="p-8 bg-[#08B36A] text-white flex justify-between items-center">
              <div>
                <h2 className="text-xl font-black uppercase tracking-tight leading-none">Nursing Mission Brief</h2>
                <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest mt-2">Agency Reference: {selectedOrder.id}</p>
              </div>
            ...
            <div className="p-10 space-y-8">
                <div className="grid grid-cols-2 gap-8">
                    <InfoItem icon={<FaIdBadge/>} label="Service ID" val={selectedOrder.id} />
                    <InfoItem icon={<FaCalendarAlt/>} label="Scheduled Date" val={selectedOrder.date} />
                    <InfoItem icon={<FaHome/>} label="Nursing Agency" val={selectedOrder.agency} />
                    <InfoItem icon={<FaClock/>} label="Visit Time" val={selectedOrder.time} />
                </div>
                ...
                <div className="bg-blue-50/50 p-6 rounded-[1.5rem] border border-blue-100/50 flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest leading-none mb-1.5">Required Service</p>
                ...
                <div className="flex items-center gap-3 justify-center py-2 bg-emerald-50 rounded-2xl p-4 border border-emerald-100">
                    ...
                    <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest">Dispatch Authorization: Verified & {selectedOrder.status}</span>
                </div>
            </div>
