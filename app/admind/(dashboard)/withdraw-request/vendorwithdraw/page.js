...
import { 
    ...
    FaExclamationTriangle // <-- Ye import add kiya hai
} from 'react-icons/fa'
...
export default function VendorWithdrawRequest() {
    ...
    const [withdrawRequests, setWithdrawRequests] = useState([
        { 
            ...
            vendorName: "newlab", 
            vendorType: "Lab", 
            accHolderName: "Newlab", 
            accNumber: "120006734098", 
            ifscCode: "12456777735574535", 
            amount: 230890.14, 
            status: "Pending" 
        },
        { 
            id: 2, 
            vendorName: "city_hospital", 
            vendorType: "Hospital", 
            accHolderName: "City Hospital Trust", 
            accNumber: "987654321000", 
    ...
    // Change Status Handler (Approve / Reject)
    const updateStatus = (e, id, newStatus) => {
        ...
        // If info modal is currently open and we update status from inside it
    ...
    const getStatusColor = (status) => {
        switch(status) {
            case 'Pending': return 'text-red-500 bg-red-50'; // Matching screenshot
            case 'Approved': return 'text-[#08B36A] bg-[#e6f7eb]';
            ...
            <div className="max-w-[1400px] mx-auto bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-5 md:p-6 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-4">
                    <div className="bg-[#e6f7eb] p-3 md:p-4 rounded-xl border border-[#08B36A]/20">
                        <FaMoneyCheckAlt className="text-[#08B36A] text-xl md:text-2xl" />
                    </div>
                    <div>
                        <h1 className="text-xl md:text-2xl font-bold text-gray-800 tracking-wide">Vendor Withdraw Request</h1>
                        <p className="text-[13px] text-gray-500 font-medium mt-0.5">Manage and process vendor payment withdrawals</p>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[1100px]">
                        <thead>
                            <tr className="bg-white border-b border-gray-100 text-[13px] text-gray-500 font-bold tracking-wide">
                                ...
                                <th className="p-5">
                                    <div className="flex items-center gap-2 cursor-pointer hover:text-[#08B36A]">Vendor Name <FaSort className="text-gray-300" /></div>
                                </th>
                                <th className="p-5">
                                    <div className="flex items-center gap-2 cursor-pointer hover:text-[#08B36A]">Vendor Type <FaSort className="text-gray-300" /></div>
                                </th>
                                <th className="p-5">
                            ...
                            {currentItems.length > 0 ? (
                                currentItems.map((item, index) => (
                                    <tr key={item.id} className="border-b border-gray-50 hover:bg-[#f8fcf9] transition-colors group">
                                        ...
                                        <td onClick={() => openInfoModal(item)} className="p-5 font-bold text-gray-800 cursor-pointer group-hover:text-[#08B36A] transition-colors">{item.vendorName}</td>
                                        <td onClick={() => openInfoModal(item)} className="p-5 font-medium text-gray-600 cursor-pointer">{item.vendorType}</td>
                                        ...
                                        <td onClick={() => openInfoModal(item)} className="p-5 font-bold text-gray-800 cursor-pointer">
                                        ...
                                        <td className="p-5 text-center">
                                            <div className="flex justify-center items-center gap-2.5">
                                                ...
                                                {item.status === 'Pending' && (
                                                    <>
                                                        <button onClick={(e) => updateStatus(e, item.id, 'Approved')} className="text-[#08B36A] hover:text-white bg-[#e6f7eb] hover:bg-[#08B36A] p-2 rounded-lg transition-all shadow-sm" title="Approve Request">
                                                            <FaCheckCircle size={14} />
                                                        </button>
                                                        <button onClick={(e) => updateStatus(e, item.id, 'Rejected')} className="text-red-500 hover:text-white hover:bg-red-500 bg-red-50 p-2 rounded-lg transition-all shadow-sm" title="Reject Request">
                        ...
                        <div className="p-6 md:p-8 overflow-y-auto">
                            ...
                            <div className="bg-gradient-to-br from-[#08B36A] to-[#04824b] rounded-2xl p-6 md:p-8 text-white flex flex-col items-center justify-center text-center shadow-lg relative overflow-hidden mb-8">
                                ...
                                <div className="mt-4 relative z-10">
                                    <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[13px] font-bold shadow-sm ${
                                        ...
                                        selectedItem.status === 'Approved' ? 'bg-white text-[#08B36A]' : 
                                        'bg-red-500 text-white'
                                    }`}>
                                        {selectedItem.status === 'Pending' && <FaExclamationTriangle size={12} />}
                                        {selectedItem.status === 'Approved' && <FaCheckCircle size={12} />}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#fafafa] p-6 rounded-2xl border border-gray-100">
                                ...
                                <div className="space-y-6 md:pr-6 md:border-r border-gray-200">
                                    <h4 className="text-[15px] font-black text-gray-800 flex items-center gap-2 border-b border-gray-200 pb-2 mb-4">
                                        <FaBuilding className="text-[#08B36A]" /> Vendor Details
                                    </h4>
                                    
                                    <div>
                                        <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wider mb-1">Vendor Name</p>
                                        <p className="text-[15px] font-bold text-gray-800">{selectedItem.vendorName}</p>
                                    </div>
                                    
                                    <div>
                                        <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wider mb-1">Vendor Type</p>
                                        <span className="inline-block px-3 py-1 bg-gray-100 rounded-md text-[12px] font-bold text-gray-600 border border-gray-200">
                            ...
                            {selectedItem.status === 'Pending' && (
                                <div className="mt-8 flex gap-4 pt-6 border-t border-gray-100">
                                    ...
                                    <button 
                                        onClick={(e) => updateStatus(e, selectedItem.id, 'Approved')}
