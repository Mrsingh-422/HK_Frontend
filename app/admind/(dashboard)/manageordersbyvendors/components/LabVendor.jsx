...
function LabVendor() {
  ...
  useEffect(() => {
    const fetchLabs = async () => {
      setLoading(true);
      try {
        const response = await AdminAPI.getAllApprovedLabs(currentPage, limit);
        
        if (response && response.success) {
          setLabs(response.data || []);
          setTotalPages(response.totalPages || 1);
        } else {
          setError("Failed to fetch approved labs.");
        }
      } catch (err) {
  ...
  const handleViewOrders = (labId) => {
    router.push(`/admind/manageordersbyvendors/viewlaborders/${labId}`);
  };
  ...
  return (
    <div className="bg-[#FAFBFD] min-h-screen text-slate-600 antialiased">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Approved Lab Vendors</h1>
          <p className="text-sm text-slate-500 mt-1">Manage, verify, and track orders across active laboratory channels</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(15,23,42,0.03)] border border-slate-200/60 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-24 text-slate-400 text-sm font-medium">
            No approved clinical labs registered yet.
          </div>
        ) : (
          /* Data Table */
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                {labs.map((lab) => {
                  const imageUrl = getProfileImageUrl(lab.profileImage);
                  
                  return (
                    <tr key={lab._id} className="hover:bg-slate-50/60 transition-colors group">
                      ...
                      <td className="p-4 font-bold text-slate-900 group-hover:text-emerald-600 transition-colors">{lab.name}</td>
                      <td className="p-4 text-slate-500 font-medium">{lab.email}</td>
                      <td className="p-4 text-slate-500 font-mono text-xs">{lab.phone}</td>
                      <td className="p-4 text-slate-500 font-medium">{lab.city || 'N/A'}</td>
                      ...
                      <td className="p-4 text-center pr-6">
                        <button
                          onClick={() => handleViewOrders(lab._id)}
