...
function HospitalVendor() {
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      ...
      setError(null);
      try {
        const response = await AdminAPI.getHospitalsInAdmin(currentPage, limit);
  ...
  const handleViewOrders = (hospitalId) => {
    router.push(`/admind/manageordersbyvendors/viewhospitalorders/${hospitalId}`);
  };
  ...
  // Helper function to safely parse and build standard URL from the hospitalImage array
  const getProfileImageUrl = (imageArray) => {
    if (!imageArray || !Array.isArray(imageArray) || imageArray.length === 0) return null;
    
    const targetPath = imageArray[0]; // Safely grab first index path element
  ...
  return (
    <div className="bg-[#FAFBFD] min-h-screen text-slate-600 antialiased p-4 md:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Approved Hospital Vendors</h1>
          <p className="text-sm text-slate-500 mt-1">Manage, verify, and track orders across active clinical hospital facilities</p>
        </div>
        <div className="text-emerald-700 bg-emerald-50 border border-emerald-200/60 px-4 py-2 rounded-xl text-xs font-bold shadow-sm tracking-wide flex items-center gap-2">
          ...
          Total Active Hospitals: {hospitals.length}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(15,23,42,0.03)] border border-slate-200/60 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-24 text-slate-400 text-sm font-medium">
            No approved medical hospitals registered yet.
          </div>
        ) : (
          /* Data Table */
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                {hospitals.map((hospital) => {
                  const imageUrl = getProfileImageUrl(hospital.hospitalImage);
                  
                  return (
                    <tr key={hospital._id} className="hover:bg-slate-50/60 transition-colors group">
                      ...
                      <td className="p-4 font-medium">
                        <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${
                          hospital.type?.toLowerCase() === 'govt' 
                            ? 'bg-blue-50 text-blue-700' 
                            : hospital.type?.toLowerCase() === 'charity'
                      ...
                      <td className="p-4 text-slate-500 font-medium">{hospital.city || 'N/A'}</td>
                      <td className="p-4 text-slate-500 font-medium">{hospital.state || 'N/A'}</td>
                      ...
                      <td className="p-4 text-center">
                        <span className="px-2.5 py-1 inline-flex text-[11px] font-extrabold tracking-wide rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/50 uppercase">
                          {hospital.profileStatus || 'Approved'}
                        </span>
                      </td>
                      ...
                      <td className="p-4 text-center pr-6">
                        <button
                          onClick={() => handleViewOrders(hospital._id)}
