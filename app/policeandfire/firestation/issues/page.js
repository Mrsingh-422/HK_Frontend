...
import api from "@/app/services/FireStationAPI";

const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || "https://hk-backend-9jm8.onrender.com";

const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Fire Station" },
...
export default function FireStationIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "Urgent",
    platform: "Web",
    appVersion: "Local Fire Station Portal",
  });
  ...
  const fireStationCategorySuggestions = [
    "Fire Engine GPS Unit Offline",
    "Emergency SOS Siren Notification Delay",
    "Water Hydrant Location Mismatch",
    "Firefighter Duty Attendance Glitch",
    "Inter-Station Call Escalation Delay",
    "Incident Report Submission Error",
    "App & Technical Bug",
  ];
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      console.error("Fire Station fetch error:", err);
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Fire Station authentication session expired. Please re-login.", "error");
      } else {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Fire Station Operations");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Fire Station Web Portal / Next.js");
      ...
      if (resData?.success) {
        showAlert("Fire Station ticket logged successfully to Technical Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <Flame className="text-[#08B36A]" size={28} /> Fire Station Incident & Support Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Log emergency dispatch glitches, fire engine GPS issues, and hydrant radar mapping bugs.
              </p>
            </div>
          </div>

          {/* Tab Switcher (Uses exact #08B36A brand color) */}
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            ...
            >
              <ClipboardList size={14} /> My Station Tickets
            </button>
            <button
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for Fire Station session. Please re-login.</span>
            </div>
            <button
              onClick={fetchIssues}
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching Fire Station incident records...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                ...
                <p className="text-xs text-slate-400">No active support tickets logged under Fire Station.</p>
                <button
                  onClick={() => setActiveTab("create")}
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
                <span>Total Station Tickets: {totalRecords} | Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <Flame className="text-[#08B36A]" size={20} /> Report Fire Station System Anomaly
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit SOS dispatch siren latency, vehicle GPS tracker issues, or hydrant radar mapping bugs.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Siren broadcast lag on active emergency response call"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Fire Station Presets:
                </span>
                {fireStationCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
            ...
            <div>
              ...
              >
                <option value="Urgent">Urgent (Emergency response affected)</option>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include vehicle call signs, station IDs, or exact error prompt..."
