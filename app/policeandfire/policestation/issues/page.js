...
const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Police Station" },
...
export default function PoliceStationIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "High",
    platform: "Web",
    appVersion: "Local Police Station Portal",
  });
  ...
  const policeStationCategorySuggestions = [
    "Patrol Vehicle GPS Offline",
    "Emergency Call Routing Latency",
    "Duty Officer Assignment Glitch",
    "Local FIR / Incident Log Sync Error",
    "Ambulance Transfer Alert Delay",
    "App & Technical Bug",
  ];
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await PoliceAPI.getMyIssuespolicestation(params);
      ...
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Police Station session expired. Please re-login.", "error");
      } else {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Police Station Operations");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Police Station Portal / Next.js");

      files.forEach((file) => {
        data.append("attachments", file);
      });

      const res = await PoliceAPI.createIssuepolicestation(data);
      const resData = res?.data || res;
      if (resData?.success) {
        showAlert("Police Station ticket logged successfully to Technical Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await PoliceAPI.trackIssuepolicestation(target);
        const resData = res?.data || res;
      ...
      const listRes = await PoliceAPI.getMyIssuespolicestation({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await PoliceAPI.trackIssuepolicestation(issueItem._id);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <Shield className="text-[#08B36A]" size={28} /> Police Station Incident & Support Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Log patrol vehicle tracking issues, emergency dispatch delays, and monitor active technical support.
              </p>
            </div>
          </div>
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for Police Station session. Please re-login.</span>
            </div>
            <button
              onClick={fetchIssues}
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching Police Station incident records...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                ...
                <p className="text-xs text-slate-400">No active support tickets logged under this station.</p>
                <button
                  onClick={() => setActiveTab("create")}
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <Shield className="text-[#08B36A]" size={20} /> Report Police Station Anomaly
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit patrol vehicle tracking glitches, emergency call routing latency, or incident log sync bugs.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Patrol vehicle #03 offline during active emergency call"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Station Presets:
                </span>
                {policeStationCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
