...
import NotificationDrawer from '../components/NotificationDrawer' // 👈 DRAWER IMPORT KIYA

export default function TopbarPoliceStation({ onToggleSidebar, isCollapsed }) {
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [isLogoutModalOpen, setIsLogoutModalOpen] = useState(false); 
  const [isNotifDrawerOpen, setIsNotifDrawerOpen] = useState(false); // 👈 DRAWER STATE
    ...
    <>
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 h-20 flex items-center justify-between px-6 lg:px-8 sticky top-0 z-40">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          <button 
            onClick={() => setIsNotifDrawerOpen(true)} // 👈 OPEN DRAWER ON CLICK
            className="p-3 text-slate-300 hover:text-[#08B36A] hover:bg-slate-50 rounded-2xl transition-all relative group"
          >
              <FaBell size={20} />
              {hasNotifications && (
          ...
          >
            <div className="text-right hidden md:block">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter truncate max-w-[150px]">
                {stationData ? `${stationData.stationCode} • ${stationData.jurisdictionArea || 'Zone'}` : "Station Officer"}
              </p>
            </div>
            ...
            <IoChevronDown size={14} className={`text-slate-400 mr-1 transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {isDropdownOpen && (
            <>
              ...
              <div className="absolute top-full right-0 mt-3 w-56 bg-white border border-gray-100 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] py-3 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
                <Link href="/policeandfire/policestation/profile" className="flex items-center gap-3 px-5 py-3 text-sm font-bold text-slate-600 hover:bg-green-50 hover:text-[#08B36A] transition-colors" onClick={() => setIsDropdownOpen(false)}>
                  <FaUserAlt size={13} /> Station Profile
                </Link>
                <div className="h-px bg-slate-50 my-2 mx-4"></div>
                <button className="w-full text-left flex items-center gap-3 px-5 py-3 text-sm font-black text-red-500 hover:bg-red-50 transition-colors" onClick={() => { setIsDropdownOpen(false); setIsLogoutModalOpen(true); }}>
                  <IoLogOutOutline size={18} /> End Shift / Logout
                </button>
              </div>
            </>
          )}
      ...
      {isLogoutModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setIsLogoutModalOpen(false)}></div>
          <div className="relative bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-300">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-slate-500 font-medium leading-relaxed">Are you sure you want to end your shift and logout from the Police Station portal?</p>
              <div className="grid grid-cols-2 gap-3 w-full mt-8">
                <button onClick={() => setIsLogoutModalOpen(false)} className="py-3.5 px-6 rounded-2xl bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">Cancel</button>
                <button onClick={handleConfirmLogout} className="py-3.5 px-6 rounded-2xl bg-red-500 text-white text-[10px] font-black uppercase tracking-widest hover:bg-red-600 shadow-lg shadow-red-200 transition-all active:scale-95">Logout</button>
              </div>
            </div>
          </div>
        </div>
