...
export default function FreshCasePage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
                ...
                <tr key={item._id} onClick={() => handleOpenDetails(item)} className="hover:bg-slate-50/50 transition-colors group cursor-pointer">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-1 text-slate-500">
                      <div className="flex items-center gap-2 text-xs font-bold"><div className={`w-1.5 h-1.5 rounded-full ${item.status === 'Pending' ? 'bg-orange-400' : 'bg-[#08B36A]'}`}></div>{item.status}</div>
                      <div className="flex items-center gap-2 text-[10px] font-medium"><FaClock size={10} className="text-slate-300" /> {formatTime(item.reportedAt)}</div>
                    </div>
                  </td>
                  <td className="px-6 py-5">
            ...
            <div className="relative bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-8 border-b border-slate-50 flex justify-between items-start">
                    <div className="flex items-center gap-4">
                        <div className="p-4 bg-red-50 text-red-500 rounded-2xl"><FaShieldAlt size={24} /></div>
                        <div>
                            <h3 className="text-2xl font-black text-slate-800 tracking-tight">Case Registry: {selectedCase.caseNo}</h3>
                            <p className="text-red-500 font-bold text-[10px] uppercase tracking-[0.15em] mt-1 flex items-center gap-2"><FaExclamationTriangle /> {selectedCase.severity} Severity Alert</p>
                        </div>
                    </div>
      ...
      {/* REFACTORED DEPLOY COMPONENT */}
      <DeployOfficerModal
        isOpen={isDeployModalOpen} 
        onClose={() => setIsDeployModalOpen(false)} 
        selectedCase={selectedCase} 
        officers={officers} 
        onDispatchSuccess={fetchData} 
      />
    </div>
  )
}
...
function CompactStatCard({ title, count, label, color, icon }) {
    const colors = { blue: "text-blue-600 bg-blue-50 border-blue-100", red: "text-red-600 bg-red-50 border-red-100", emerald: "text-emerald-600 bg-emerald-50 border-emerald-100" }
    return (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all duration-300">
            <div className="relative z-10 flex items-center gap-5">
                <div className={`w-12 h-12 shrink-0 rounded-xl flex items-center justify-center ${colors[color]} border shadow-inner`}>{React.cloneElement(icon, {size: 18})}</div>
                <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{title}</p><div className="flex items-baseline gap-2"><h2 className={`text-4xl font-black tracking-tight ${colors[color].split(' ')[0]}`}>{count}</h2><span className="text-[10px] font-bold text-slate-300 uppercase">{label}</span></div></div>
            </div>
        </div>
    )
}
...
function InfoItem({ label, value, color = "text-slate-700" }) {
    return (
        <div><p className="text-[9px] font-black text-slate-400 uppercase ml-1 mb-1">{label}</p><div className="bg-white border border-slate-100 px-4 py-2.5 rounded-xl"><p className={`text-sm font-bold ${color} line-clamp-2`}>{value}</p></div></div>
    )
}