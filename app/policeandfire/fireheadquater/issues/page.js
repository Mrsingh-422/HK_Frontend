...
import api from "@/app/services/FireHeadAPI";

const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || "https://hk-backend-9jm8.onrender.com";

const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Fire HQ" },
...
export default function FireHQIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "Urgent",
    platform: "Web",
    appVersion: "Fire HQ Command Portal",
  });
  ...
  const fireHQCategorySuggestions = [
    "Emergency SOS Dispatch Broadcast Failure",
    "City-Wide Fire Station Sync Glitch",
    "Hydrant GPS Mapping Error",
    "Inter-Department Police Escalation Lag",
    "Emergency Route Radar Jamming",
    "Fire Engine Unit Offline Alert",
    "App & Technical Bug",
  ];
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      console.error("Fire HQ fetch error:", err);
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Fire HQ authentication session expired or missing token. Please re-login.", "error");
      } else {
        showAlert(err.response?.data?.message || "Failed to load incident tickets", "error");
      }
    } finally {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Fire Department Operations");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Fire HQ Command Portal / Next.js");
      ...
      if (resData?.success) {
        showAlert("Fire HQ incident ticket logged successfully to 24/7 Technical Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
      ...
      showAlert(err.response?.data?.message || "Failed to submit ticket. Please check session.", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await api.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await api.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await api.trackIssue(issueItem._id);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {alertBanner && (
          <div className={`p-4 rounded-2xl flex items-center justify-between text-xs font-bold shadow-lg animate-in fade-in duration-200 ${
            alertBanner.type === "error" ? "bg-rose-50 border border-rose-200 text-rose-800" : "bg-orange-50 border border-orange-200 text-orange-800"
          }`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <Flame className="text-orange-600" size={28} /> Fire Headquarter Incident & Support Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Log emergency dispatch glitches, city-wide station sync errors, and track 24/7 technical fixes.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "list" ? "bg-orange-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <ClipboardList size={14} /> My HQ Tickets
            </button>
            <button
              onClick={() => setActiveTab("create")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "create" ? "bg-orange-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <Plus size={14} /> Log Emergency Issue
            </button>
            <button
              onClick={() => setActiveTab("track")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "track" ? "bg-orange-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for Fire HQ session. Please re-login.</span>
            </div>
            <button
              onClick={fetchIssues}
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-orange-500"
                />
              </div>
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching Fire HQ incident records...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                ...
                <p className="text-xs text-slate-400">No active support tickets logged under Fire HQ.</p>
                <button
                  onClick={() => setActiveTab("create")}
                  className="bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow"
                >
                  Log Emergency Issue
                </button>
              </div>
                  ...
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-[11px] font-black text-orange-700 bg-orange-50 px-2 py-0.5 rounded border border-orange-100">
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
                <span>Total HQ Tickets: {totalRecords} | Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <Flame className="text-orange-600" size={20} /> Report Fire Department System Anomaly
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit SOS dispatch broadcast latency, station sync errors, or hydrant radar mapping bugs.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Fire Engine Unit #12 GPS radar latency on live dispatch"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 mb-2"
              />
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Fire HQ Presets:
                </span>
                {fireHQCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => setFormData({ ...formData, category: cat })}
                    className="px-2 py-0.5 bg-slate-100 hover:bg-orange-50 hover:text-orange-700 text-slate-600 rounded-md text-[10px] font-bold transition"
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-700 outline-none cursor-pointer focus:border-orange-500"
              >
                <option value="Urgent">Urgent (Emergency dispatch affected)</option>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include incident logs, vehicle call signs, station IDs, or exact error prompt..."
                value={formData.detailedDescription}
                onChange={(e) => setFormData({ ...formData, detailedDescription: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-700 outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500"
              />
            </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-slate-200 hover:border-orange-500 rounded-2xl p-5 text-center cursor-pointer bg-slate-50/50 transition relative">
                <input
                  type="file"
                  multiple
                  accept="image/*,.pdf"
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-orange-600 hover:bg-orange-700 text-white px-8 py-3 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-2 disabled:opacity-50"
              >
                {submitting ? <Loader2 size={16} className="animate-spin" /> : null}
                Submit Incident Issue
              </button>
            </div>
          </form>
        )}
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-orange-500"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loadingTrack}
                  className="bg-orange-600 hover:bg-orange-700 text-white px-6 py-3 rounded-2xl text-xs md:text-sm font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
              ...
              <div className="space-y-6 animate-in zoom-in-95 duration-200">
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-black text-orange-700 bg-orange-50 px-2.5 py-1 rounded-lg border border-orange-100">
                ...
                {trackingData.resolutionDetails?.resolutionNote && (
                  <div className="bg-orange-50 border border-orange-200 p-6 rounded-3xl space-y-2">
                    ...
                    {trackingData.resolutionDetails.resolvedAt && (
                      <span className="text-[11px] text-orange-700 font-mono block pt-1">
                ...
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-6">
                  <h3 className="font-black text-base text-slate-900 tracking-tight flex items-center gap-2">
                    <Clock size={18} className="text-orange-600" /> Resolution Audit Timeline
                  </h3>
                    ...
                    <div className="relative border-l-2 border-orange-500/30 ml-4 pl-6 space-y-6">
                      {trackingData.timeline.map((entry, idx) => (
                        <div key={idx} className="relative space-y-1">
                          <div className="w-3.5 h-3.5 bg-orange-600 ring-4 ring-orange-100 rounded-full absolute -left-[31px] top-1" />
                          <div className="flex flex-wrap items-center gap-2">
