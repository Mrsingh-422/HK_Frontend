...
const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Police HQ" },
...
export default function PoliceHQIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "Urgent",
    platform: "Web",
    appVersion: "Police HQ Master Command Portal",
  });
  ...
  const policeHQCategorySuggestions = [
    "City-Wide PCR Radar Tracking Glitch",
    "Emergency SOS Alert Broadcast Delay",
    "Inter-Agency Fire/Ambulance Escalation Error",
    "Patrol Vehicle GPS Drift",
    "Police HQ Inbound Call Routing Issue",
    "App & Technical Bug",
  ];
  ...
  // 3.2 FETCH ISSUES LIST (Fixed to PoliceAPI)
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await PoliceAPI.getMyIssues(params);
      ...
      console.error("Police HQ fetch error:", err);
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Police HQ authentication session expired. Please re-login.", "error");
      } else {
  ...
  // 3.1 SUBMIT NEW ISSUE (Fixed to PoliceAPI)
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Police HQ Dispatch Operations");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Police HQ Command Portal / Next.js");

      files.forEach((file) => {
        data.append("attachments", file);
      });

      const res = await PoliceAPI.createIssue(data);
      const resData = res?.data || res;
      if (resData?.success) {
        showAlert("Police HQ ticket logged successfully to Technical Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
  ...
  // 3.3 TRACK TICKET (Fixed to PoliceAPI)
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await PoliceAPI.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await PoliceAPI.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await PoliceAPI.trackIssue(issueItem._id);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <ShieldAlert className="text-[#08B36A]" size={28} /> Police Headquarter Incident & Support Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Log city-wide PCR radar tracking glitches, SOS alert broadcast delays, and track technical fixes.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "list" ? "bg-[#08B36A] text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "create" ? "bg-[#08B36A] text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "track" ? "bg-[#08B36A] text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for Police HQ session. Please re-login.</span>
            </div>
            <button
              onClick={fetchIssues}
              className="px-3 py-1.5 bg-[#08B36A] text-white rounded-lg hover:bg-[#079b5c] transition"
            >
              Retry
            </button>
          </div>
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching Police HQ incident records...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                ...
                <p className="text-xs text-slate-400">No active support tickets logged under Police HQ.</p>
                <button
                  onClick={() => setActiveTab("create")}
                  className="bg-[#08B36A] hover:bg-[#079b5c] text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow"
                >
                  Log Emergency Issue
                </button>
              </div>
                  ...
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-[11px] font-black text-[#08B36A] bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <ShieldAlert className="text-[#08B36A]" size={20} /> Report Police Department System Anomaly
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit PCR radar tracking glitches, emergency broadcast latency, or patrol vehicle map bugs.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. PCR Unit #07 GPS radar signal drift on active emergency response"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-[#08B36A]/20 focus:border-[#08B36A]"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-[#08B36A]/20 focus:border-[#08B36A] mb-2"
              />
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Police HQ Presets:
                </span>
                {policeHQCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => setFormData({ ...formData, category: cat })}
                    className="px-2 py-0.5 bg-slate-100 hover:bg-emerald-50 hover:text-[#08B36A] text-slate-600 rounded-md text-[10px] font-bold transition"
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-700 outline-none cursor-pointer focus:border-[#08B36A]"
              >
                <option value="Urgent">Urgent (Active emergency response affected)</option>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include patrol vehicle IDs, sector coordinates, or exact error prompt..."
                value={formData.detailedDescription}
                onChange={(e) => setFormData({ ...formData, detailedDescription: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-700 outline-none focus:ring-2 focus:ring-[#08B36A]/20 focus:border-[#08B36A]"
              />
            </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-slate-200 hover:border-[#08B36A] rounded-2xl p-5 text-center cursor-pointer bg-slate-50/50 transition relative">
                <input
                  type="file"
                  multiple
                  accept="image/*,.pdf"
              ...
              {files.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {files.map((f, i) => (
                    ...
                    >
                      <FileText size={12} className="text-[#08B36A]" />
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-[#08B36A]"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loadingTrack}
                  className="bg-[#08B36A] hover:bg-[#079b5c] text-white px-6 py-3 rounded-2xl text-xs md:text-sm font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
