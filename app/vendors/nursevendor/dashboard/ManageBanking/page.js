...
import { 
  Landmark, User, CreditCard, CheckCircle, 
  AlertTriangle, Shield, Save, Loader2, ChevronDown 
} from 'lucide-react';
...
export default function ManageBankingPage() {
  ...
  // 1. Fetch bank details from the Wallet Stats API
  const fetchBankDetails = async () => {
    setFetching(true);
    try {
      const response = await NurseAPI.getWalletStats();
      ...
      console.error("Error loading nurse bank settings", error);
  ...
  // 2. Update bank details using the dedicated Wallet Patch endpoint
  const handleSubmit = async (e) => {
    ...
    const bankPayload = {
      ...
      upiId: bankDetails.upiId || undefined // Send as undefined if blank to maintain schema standards
    };

    try {
      // Submits as clean JSON payload direct to /provider/wallet/bank-details
      const res = await NurseAPI.updateBankDetails(bankPayload);
      if (res.success) {
        toast.success(res.message || "Bank details successfully saved. Payouts are locked until verification.");
        await fetchBankDetails(); // Refresh updated states and verified status
      } else {
  ...
  return (
    <div className="w-full mx-auto pb-20 px-4 font-sans">
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-gradient-to-br from-[#1e3a8a] to-blue-900 rounded-[2.5rem] p-6 text-white shadow-xl relative overflow-hidden flex flex-col justify-between aspect-[1.58/1]">
            ...
            <div className="flex justify-between items-start z-10">
              <Landmark size={28} className="text-[#08B36A]" />
              {/* Card Status Badge */}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 md:col-span-2 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-50 pb-3">
            <h3 className="text-lg font-black text-gray-800 flex items-center gap-2 uppercase tracking-tighter">
              <CreditCard className="text-[#08B36A]" size={20} /> Update Bank Settlement Account
            </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="sm:col-span-2 space-y-1">
              ...
              <input 
                ...
                onChange={handleInputChange} 
                className="input-style font-mono"
                placeholder="e.g. nurseaccount@okaxis"
              />
            </div>
          </div>
          ...
          <div className="flex justify-end pt-4 border-t border-gray-50">
            ...
            >
              {saving ? <><Loader2 className="animate-spin" size={16} /> Saving Account...</> : <><Save size={16} /> Save Account Info</>}
            </button>
          </div>
        </form>
