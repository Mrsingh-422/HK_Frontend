...
const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Nurse" },
...
export default function NurseIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "High",
    platform: "Web",
    appVersion: "Nurse Home Care Portal",
  });
  ...
  const nurseCategorySuggestions = [
    "Home Visit Geofence & Check-In Error",
    "Patient Vitals Form Sync Failure",
    "Nurse Booking Schedule Overlap",
    "Travel Allowance & Payout Discrepancy",
    "Emergency Patient Escalation Lag",
    "Medication Administration Log Glitch",
    "Patient Address / Map Navigation Issue",
    "App & Technical Bug",
  ];
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await NurseAPI.getMyIssues(params);
      ...
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Nurse session expired or missing token. Please log in again.", "error");
      } else {
        showAlert(err.response?.data?.message || "Failed to load nurse tickets", "error");
      }
    } finally {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Nursing Service Support");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Nurse Care Web Portal / Next.js");

      files.forEach((file) => {
        data.append("attachments", file);
      });

      const res = await NurseAPI.createIssue(data);
      const resData = res?.data || res;
      if (resData?.success) {
        showAlert("Nursing duty ticket logged successfully to 24/7 Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
      ...
      showAlert(err.response?.data?.message || "Failed to submit ticket. Please check nurse login session.", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await NurseAPI.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await NurseAPI.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await NurseAPI.trackIssue(issueItem._id);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {alertBanner && (
          <div className={`p-4 rounded-2xl flex items-center justify-between text-xs font-bold shadow-lg animate-in fade-in duration-200 ${
            alertBanner.type === "error" ? "bg-rose-50 border border-rose-200 text-rose-800" : "bg-rose-50 border border-rose-200 text-rose-800"
          }`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <HeartPulse className="text-rose-600" size={28} /> Nursing Duty & Home Care Support Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Report patient vitals form sync errors, geofencing check-in glitches, and schedule overlaps.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "list" ? "bg-rose-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <ClipboardList size={14} /> My Duty Tickets
            </button>
            <button
              onClick={() => setActiveTab("create")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "create" ? "bg-rose-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <Plus size={14} /> Log Home Care Issue
            </button>
            <button
              onClick={() => setActiveTab("track")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "track" ? "bg-rose-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for your nurse session. Please re-login.</span>
            </div>
            <button
              onClick={fetchIssues}
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-rose-500"
                />
              </div>
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching nursing incident records...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                <AlertTriangle className="mx-auto text-slate-300 w-12 h-12" />
                <h3 className="text-base font-bold text-slate-700">No Duty Tickets Found</h3>
                <p className="text-xs text-slate-400">No active incidents reported under your nurse account.</p>
                <button
                  onClick={() => setActiveTab("create")}
                  className="bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow"
                >
                  Log A Home Care Anomaly
                </button>
              </div>
            ) : (
                  ...
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-[11px] font-black text-rose-700 bg-rose-50 px-2 py-0.5 rounded border border-rose-100">
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
                <span>Total Nurse Tickets: {totalRecords} | Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <HeartPulse className="text-rose-600" size={20} /> Report Nursing Visit or Task Anomaly
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit patient vitals form sync errors, geofencing check-in glitches, or schedule overlaps.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Geofence check-in fails on patient home visit arrival"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Type custom category (e.g. Vitals Form Lag, Nurse Travel Allowance)..."
                value={formData.category}
                onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 mb-2"
              />
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Nursing Presets:
                </span>
                {nurseCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => setFormData({ ...formData, category: cat })}
                    className="px-2 py-0.5 bg-slate-100 hover:bg-rose-50 hover:text-rose-700 text-slate-600 rounded-md text-[10px] font-bold transition"
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-700 outline-none cursor-pointer focus:border-rose-500"
              >
                <option value="Urgent">Urgent (Emergency home care visit blocked)</option>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include booking IDs, patient addresses, vitals recorded, or exact system error prompts..."
                value={formData.detailedDescription}
                onChange={(e) => setFormData({ ...formData, detailedDescription: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-700 outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500"
              />
            </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-slate-200 hover:border-rose-500 rounded-2xl p-5 text-center cursor-pointer bg-slate-50/50 transition relative">
                <input
                  type="file"
                  multiple
                  accept="image/*,.pdf"
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-rose-600 hover:bg-rose-700 text-white px-8 py-3 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-2 disabled:opacity-50"
              >
                {submitting ? <Loader2 size={16} className="animate-spin" /> : null}
                Submit Nursing Issue
              </button>
            </div>
          </form>
        )}
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-rose-500"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loadingTrack}
                  className="bg-rose-600 hover:bg-rose-700 text-white px-6 py-3 rounded-2xl text-xs md:text-sm font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
              ...
              <div className="space-y-6 animate-in zoom-in-95 duration-200">
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-black text-rose-700 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-100">
                ...
                {trackingData.resolutionDetails?.resolutionNote && (
                  <div className="bg-rose-50 border border-rose-200 p-6 rounded-3xl space-y-2">
                    ...
                    {trackingData.resolutionDetails.resolvedAt && (
                      <span className="text-[11px] text-rose-700 font-mono block pt-1">
                ...
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-6">
                  <h3 className="font-black text-base text-slate-900 tracking-tight flex items-center gap-2">
                    <Clock size={18} className="text-rose-600" /> Resolution Audit Timeline
                  </h3>

                  {trackingData.timeline && trackingData.timeline.length > 0 ? (
                    <div className="relative border-l-2 border-rose-500/30 ml-4 pl-6 space-y-6">
                      {trackingData.timeline.map((entry, idx) => (
                        <div key={idx} className="relative space-y-1">
                          <div className="w-3.5 h-3.5 bg-rose-600 ring-4 ring-rose-100 rounded-full absolute -left-[31px] top-1" />
                          <div className="flex flex-wrap items-center gap-2">
