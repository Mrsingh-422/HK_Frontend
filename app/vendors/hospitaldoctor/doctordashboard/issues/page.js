...
export default function DoctorIssuesPage() {
  ...
  const [doctorType, setDoctorType] = useState("Hospital Doctor"); // 'Independent Doctor' | 'Hospital Affiliated Doctor'
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await HospitalDoctorAPI.getMyIssues(params);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      const res = await HospitalDoctorAPI.createIssue(data);
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await HospitalDoctorAPI.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await HospitalDoctorAPI.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await HospitalDoctorAPI.trackIssue(issueItem._id);
