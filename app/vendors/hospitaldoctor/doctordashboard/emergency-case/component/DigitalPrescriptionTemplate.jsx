...
// Helper to safely join relative backend file paths without causing broken links [2]
...
// Defensive helper to confirm valid 24-character hexadecimal MongoDB ObjectId string [2]
const isValidObjectId = (id) => {
    if (!id || typeof id !== 'string') return false;
    return /^[0-9a-fA-F]{24}$/.test(id);
};

// Defensive resolver to safely unpack backend payloads at any nesting depth [2]
...
export default function DigitalPrescriptionTemplate({ 
    ...
    // Resolve target active ID to request from print endpoint dynamically [2]
    useEffect(() => {
        const loadPrintData = async () => {
            ...
            if (!targetId) {
                console.warn("Skipping dynamic print fetch: No valid MongoDB ObjectId found in data context.");
                return;
            }
            ...
            try {
                ...
                // Axios-Immune Cascade Resolver: sets state regardless of unwrapping interceptors [2]
                if (response) {
                ...
                console.error("Failed to fetch discharge print data dynamically:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    // Use fetched database data if loaded, otherwise cascade back to standard props data [2]
    ...
    const clinicalNotes = activePayload?.clinicalNotes || "";

    // Header Meta-details Resolvers (Static fallback text strings removed) [2]
    ...
    const mainDoctorName = leadDoctor?.name || activePayload?.mainDoctorName || "";
    const mainDoctorQualification = leadDoctor?.qualification || activePayload?.mainDoctorQualification || "";
    const mainDoctorTitle = leadDoctor?.title || activePayload?.mainDoctorTitle || "";

    const collaborativeDoctors = header?.collaborativeDoctors || activePayload?.bedsideCareTeam || [];

    // Patient Details Resolvers aligned with target payload properties [2]
    ...
    const chiefComplaints = patientDetails?.chiefComplaints || activePayload?.chiefComplaints || "";
    const diagnosis = patientDetails?.diagnosis || activePayload?.diagnosis || "";

    const dateOfAdmission = patientDetails?.dateOfAdmission || activePayload?.dateOfAdmission || "";
    const department = patientDetails?.department || activePayload?.department || "Department of Medicine, Unit - 1";
    const dateOfDischarge = patientDetails?.dateOfDischarge || activePayload?.dateOfDischarge || "";
    const dateOfSurgery = patientDetails?.dateOfSurgery || activePayload?.dateOfSurgery || "";
    ...
    const paymentType = patientDetails?.paymentType || activePayload?.paymentType || "UPI (Google Pay)";
    const conditionDuringAdmission = patientDetails?.conditionDuringAdmission || activePayload?.conditionDuringAdmission || "";
    const conditionDuringDischarge = patientDetails?.conditionDuringDischarge || activePayload?.conditionDuringDischarge || "";

    // Medications Normalization structure matching API response keys [2, 4]
    const rawMedicines = medications.length > 0 ? medications : (activePayload?.medicines || []);
    const paddedMedicines = Array.from({ length: 10 }, (_, index) => {
        const med = rawMedicines[index];
        if (!med) return null;
        return {
    ...
    // Follow Up Instructions Resolvers [2]
    const advisedInvestigations = followUp?.adviseInvestigation || activePayload?.investigations || "xxx";
    const adviceGiven = followUp?.adviceGiven || activePayload?.advice || "xxx";
    const specialInstructions = followUp?.anySpecialInstructionGiven || activePayload?.specialInstructions || "xxx";
    const nextAppointment = followUp?.nextAppointment || activePayload?.nextAppointment || "";
    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto font-sans">
            {/* Click-outside backdrop closer */}
            <div className="absolute inset-0" onClick={onClose}></div>
            
            {/* Modal Wrapper Container */}
            <div className="relative bg-white w-full max-w-[225mm] rounded-3xl shadow-2xl overflow-hidden my-8 flex flex-col p-6 sm:p-8 z-10">
                
                {/* Top Actions (Omitted from Print view) */}
                <div className="flex justify-between items-center pb-4 mb-4 border-b border-slate-100 no-print gap-3 flex-wrap">
                    ...
                    <span className="font-extrabold text-sm text-slate-800">Print / Discharge Preview</span>
                    <div className="flex items-center gap-2">
                        {/* Print/Download Button [5] */}
                        <button 
                            onClick={handlePrint}
                            disabled={loading}
                        ...
                        {/* Optional "Complete Discharge" button [2] */}
                        ...
                        {/* Optional "Complete Bedside Shift" button [2] */}
                ...
                {/* Printable Frame Area with Screen-preview Scroll limit */}
                <div className="overflow-y-auto max-h-[75vh] pr-1 no-scrollbar">
                    {loading ? (
                        ...
                        >
                            {/* Top-Left Curve Branding Accent [4] */}
                            ...
                            <div className="relative z-10">
                                ...
                                {/* Logo & Header Info Area */}
                                <div className="flex justify-between items-stretch mb-6">
                                    {/* Custom Clinic Logo Area [2] */}
                                    <div className="w-1/3 flex flex-col items-start justify-center pr-2 pl-4">
                                        <div className="relative w-36 h-20 flex flex-col items-center justify-center text-center">
                                            {hospitalLogo && (
                                                <img 
                                                    src={hospitalLogo} 
                                                    alt="Hospital Logo" 
                                    ...
                                    {/* Doctor Info Right [2] */}
                                    <div className="w-2/3 text-right pl-6 font-sans">
                                        ...
                                        <div className="mt-2 text-slate-800 leading-tight">
                                            ...
                                            {/* Dynamic Bedside Care Team Listing [2] */}
                                ...
                                <div className="mb-4 font-sans">
                                    ...
                                    {/* Two Column Patient Details Grid */}
                                    <div className="grid grid-cols-12 gap-x-8 gap-y-2 text-[10px] text-slate-700 leading-relaxed">
                                        ...
                                        <div className="col-span-6 space-y-2">
                                            ...
                                            <div className="flex items-end">
                                                <span className="w-24 font-bold flex-shrink-0 text-slate-800">Cheif Complaints</span>
                                                <span className="mr-1.5 font-bold">:</span>
                                ...
                                {/* Next Appointment & Signature space */}
                                    ...
                                    <div className="flex flex-col items-center gap-1.5">
                                        <div className="w-14 h-14 bg-white border border-slate-200 p-1 flex items-center justify-center rounded-lg shadow-sm">
                                            <svg className="w-full h-full text-slate-800" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M3 3h8v8H3zm2 2v4h4V5zm8-2h8v8h-8zm2 2v4h4V5zM3 13h8v8H3zm2 2v4h4v-4zm13-2h3v2h-3zm-2 2h2v2h-2zm2 2h3v3h-3zm-2 1h2v2h-2zm-2-3h2v2h-2zm5-1h1v1h-1z" />
                                            </svg>
                                        </div>
