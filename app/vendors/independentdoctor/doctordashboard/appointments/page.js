  ...
  const fetchData = async () => {
    try {
      ...
      const [statsRes, bookingsRes] = await Promise.all([
        ...
        DoctorAPI.getPatientBookings(apiFilter)
      ]);
      
      if (statsRes.success) setStats(statsRes.data);
      if (bookingsRes.success) setAppointments(bookingsRes.data);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-gray-200">
                <p className="text-gray-400 font-bold uppercase text-xs tracking-widest">No appointments found for this status</p>
            </div>
        ) : (
            ...
            <div className="bg-white rounded-[3.5rem] w-full max-w-2xl max-h-[90vh] overflow-hidden relative shadow-2xl animate-in fade-in zoom-in-95 duration-300 flex flex-col">
                ...
                <div className="p-8 border-t border-gray-50 flex gap-3">
                    <button 
                        ...
                        className="flex-1 py-4 rounded-2xl border border-gray-100 font-black text-[10px] uppercase tracking-widest text-gray-400 hover:bg-gray-50 transition-all"
                    >
                        Close Details
                    </button>
