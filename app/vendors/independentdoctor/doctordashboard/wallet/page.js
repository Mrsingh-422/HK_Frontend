...
export default function DoctorWalletDashboard() {
  // ==========================================
  // STATES
  // ==========================================
  ...
  const loadWalletData = useCallback(async () => {
    ...
    try {
      ...
      if (statsRes?.success) {
        ...
        setFormError(statsRes?.message || 'Failed to load wallet stats.');
      }

      // 2. Fetch Transactions
      const txRes = await DoctorAPI.getDoctorTransactions().catch(() => null);
      if (txRes?.success) {
      ...
      setFormError(err?.response?.data?.message || 'Connection error loading wallet info.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleWithdraw = async (e) => {
    ...
    if (!bank?.accountNumber) {
      setFormError('Please add bank details before requesting a payout.');
      return;
    }

    if (!bank?.isVerified) {
      setFormError('Payouts are locked. Your bank details are pending Admin verification.');
      return;
    }

    const withdrawable = stats?.withdrawableBalance ?? 0;
    if (parsedAmount > withdrawable) {
      setFormError(`Cannot exceed withdrawable balance of ₹${withdrawable.toLocaleString('en-IN')}.`);
      return;
    }
    ...
    try {
      ...
      if (res?.success) {
        setFormSuccess(
          res.message ||
            'Withdrawal request submitted successfully. Waiting for admin manual payout approval.'
        );
        setWithdrawAmount('');
  ...
  const handleBankSubmit = async (e) => {
    ...
    try {
      setBankLoading(true);
      const res = await DoctorAPI.updateBankDetails(bankForm);

      if (res?.success) {
        setModalMsg({
          type: 'success',
          text: res.message || 'Bank details updated. Payouts locked until Admin verifies.'
        });
        setTimeout(async () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-10 min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1e3a8a] flex items-center gap-2">
            <FaWallet className="text-[#08B36A]" /> Doctor Financial Wallet
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Manage your consultation earnings, admin deductions, and bank payouts.
          </p>
        </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-red-400 uppercase tracking-wider">Commission Cutoff</span>
          <p className="text-xl font-extrabold text-red-600 mt-1">- ₹{(stats?.adminCommissionDeducted || 0).toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Today Earnings</span>
          <p className="text-xl font-extrabold text-slate-800 mt-1">₹{(stats?.todayEarning || 0).toLocaleString('en-IN')}</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Weekly Earnings</span>
          <p className="text-xl font-extrabold text-slate-800 mt-1">₹{(stats?.weeklyEarning || 0).toLocaleString('en-IN')}</p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col justify-between">
          <div className="p-6">
            ...
            <p className="text-[11px] text-slate-400 mt-2">Cleared balance ready for instant bank transfer request.</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
          ...
          <div className="border-t border-slate-100 pt-4 mt-4">
            <p className="text-[11px] text-slate-400 font-semibold italic">
              7-Day lock window to handle dispute/cancellations before clearance.
            </p>
          </div>
        </div>

        {/* Card 3: Total Net Balance */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-2">
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl animate-in zoom-in duration-150">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <FaUniversity className="text-[#08B36A]" /> Update Bank Settlement Details
              </h3>
            ...
            <p className="text-[11px] text-amber-700 bg-amber-50 p-2.5 rounded-lg border border-amber-200 mb-4 font-semibold">
              🔒 Security Guard: Updating bank details will reset your verification status to Pending. Payouts will be locked until Admin approval.
            </p>
