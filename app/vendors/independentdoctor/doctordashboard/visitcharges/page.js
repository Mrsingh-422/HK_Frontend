...
export default function VisitChargesPage() {
  ...
  const fetchCharges = async () => {
    try {
      setLoading(true);
      const response = await DoctorAPI.getMyVisitCharges();
      if (response.success && response.data) {
        setChargeData(response.data);
        setFormData({
          fixedPrice: response.data.fixedPrice || 0,
          fixedDistance: response.data.fixedDistance || 0,
          pricePerKM: response.data.pricePerKM || 0,
          fastDeliveryExtra: response.data.fastDeliveryExtra || 0,
          freeDeliveryThreshold: response.data.freeDeliveryThreshold || 0,
          taxPercentage: response.data.taxPercentage || 0
        });
      }
    } catch (error) {
      console.error("Error fetching charges:", error);
      toast.error("Failed to load visit charges");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      const response = await DoctorAPI.saveVisitCharges(formData);
      if (response.success) {
        toast.success("Visit charges updated successfully");
  ...
  return (
    <div className="w-full bg-[#fcfdfe] min-h-screen pb-20 font-sans">
      ...
      <div className="bg-white border-b border-slate-200/60 top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <nav className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">
              ...
              <span className="text-slate-800">Visit Fees</span>
            </nav>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">
              Visit Charges Management
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {chargeData ? 'Update Visit Fees' : 'Set Initial Fees'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-10">
        {loading && !chargeData ? (
          <div className="bg-white rounded-3xl border border-slate-200 p-32 flex flex-col items-center justify-center">
            ...
            <p className="mt-6 text-slate-500 font-bold tracking-tight text-center">Syncing Visit Fee Configuration...</p>
          </div>
        ) : chargeData ? (
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-sm font-bold text-slate-400 uppercase mb-1">Travel Expenses</p>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-sm font-bold text-slate-400 uppercase mb-1">Included Distance</p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
                    <div className="space-y-6">
                        <div className="flex justify-between py-3 border-b border-slate-50 items-end">
                            <span className="text-slate-500 font-medium">Extra Charge (per KM)</span>
              ...
              <div className="bg-[#1e293b] rounded-3xl p-8 text-white relative overflow-hidden group">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4 text-[#08B36A]">
                        <FaInfoCircle />
                        <span className="text-xs font-black uppercase tracking-widest">Pricing Algorithm</span>
                    </div>
                    <p className="text-slate-400 text-sm leading-relaxed max-w-2xl mb-6">
                        Fees are calculated automatically based on the distance between your clinic/home location and the patient's address. 
                        Initial expenses cover the radius, while excess travel is billed per KM.
                    </p>
                    <div className="bg-black/30 p-4 rounded-xl font-mono text-xs text-green-400 inline-block border border-white/5">
                        Charge = [Travel Expenses] + (Max(0, Actual Distance - {chargeData.fixedDistance}) * {chargeData.pricePerKM})
                    </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-4">
               <div className="bg-white rounded-3xl border border-slate-200/60 p-8 sticky top-28 shadow-sm">
                  <h3 className="text-lg font-black text-slate-900 mb-6 flex items-center gap-2">
                    <FaHouseUser className="text-slate-300" /> Visit Summary
                  </h3>
                  
                  <div className="space-y-4">
                    <div className="p-4 bg-slate-50 rounded-2xl flex flex-col gap-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Initial Travel Expenses</span>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <div className="p-4 bg-slate-50 rounded-2xl">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter block mb-1">GST/Tax Rate</span>
          ...
          <div className="bg-white border-2 border-dashed border-slate-200 rounded-[3rem] p-24 text-center">
            ...
            <h3 className="text-2xl font-black text-slate-900 mb-3">No Fee Configuration</h3>
            <p className="text-slate-500 mb-8 max-w-sm mx-auto font-medium leading-relaxed">
              Your account doesn't have travel expenses set yet. Patients cannot book home visits until these rates are configured.
            </p>
            <button 
              onClick={() => setIsModalOpen(true)}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white w-full max-w-3xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-white/20">
            ...
            <div className="px-10 py-8 border-b border-slate-100 flex justify-between items-start bg-white">
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Edit Travel Rates</h2>
                <p className="text-slate-500 text-sm mt-1 font-medium">Update the variables for your travel expenses and visit fees.</p>
              </div>
            ...
            <div className="p-10 overflow-y-auto">
              <form id="visitChargeForm" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                  <div className="space-y-2 group">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1 group-focus-within:text-[#08B36A] transition-colors">Travel Expenses (₹)</label>
                  ...
                  <div className="space-y-2 group">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1 group-focus-within:text-[#08B36A] transition-colors">Distance Included (KM)</label>
                    <div className="relative">
                  ...
                  <div className="space-y-2 group">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Extra Fee / KM (₹)</label>
                  ...
                  <div className="space-y-2 group">
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">Tax (%)</label>
                    <div className="relative">
            ...
            <div className="px-10 py-8 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-4">
              ...
              <button type="submit" form="visitChargeForm" disabled={submitting} className="px-12 py-4 bg-slate-900 hover:bg-black text-white text-sm font-black rounded-2xl shadow-xl shadow-slate-200 flex items-center gap-3 transition-all active:scale-95">
                {submitting ? <FaSpinner className="animate-spin" /> : <FaSave />}
                {submitting ? 'Updating...' : 'Save Config'}
              </button>
            </div>
          </div>
        </div>
