...
export default function DoctorPromotionsPage() {
  ...
  // Form state matching your Schema Documentation
  ...
  const fetchCoupons = async () => {
    try {
      setLoading(true);
      const res = await DoctorAPI.listCoupons();
      // Handle response based on common API patterns
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await DoctorAPI.updateCoupon(editingId, payload);
        toast.success('Clinic Coupon Updated!');
      } else {
        await DoctorAPI.addCoupon(payload);
        toast.success('New Clinic Coupon Created!');
      }
      closeModal();
      fetchCoupons();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this clinic coupon?")) {
      try {
        await DoctorAPI.deleteCoupon(id);
  ...
  const handleToggleStatus = async (id) => {
    try {
      await DoctorAPI.toggleCoupon(id);
    ...
    <div className="w-full pb-20 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4 p-6">
        <div>
          ...
          <p className="text-gray-500 font-medium text-sm mt-2">Manage consultation discounts and healthcare promotional codes.</p>
        </div>
      ...
      {coupons.length > 0 ? (
                ...
                <div>
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {coupon.isAdminCreated && (
                      <span className="bg-indigo-100 text-indigo-700 text-[8px] font-black px-2 py-0.5 rounded-md border border-indigo-200">GLOBAL PROMO</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-y-2 gap-x-4 mt-4">
                     ...
                     <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400">
                        <FaHospitalUser className="text-gray-300"/> Max Cap: <span className="text-[#08B36A] font-black">₹{coupon.maxDiscount}</span>
                     </div>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-50">
                  <div className="flex items-center gap-1">
                    {!coupon.isAdminCreated ? (
                      ...
                      <span className="text-[8px] font-bold text-gray-300 uppercase tracking-widest">Admin Created</span>
                    )}
                  </div>
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-2 text-[9px] font-black px-4 py-2 rounded-xl border transition-all active:scale-95 ${coupon.isActive ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-600 border-red-200'} ${coupon.isAdminCreated ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    {coupon.isActive ? <><FaPowerOff size={10}/> ACTIVE</> : <><FaPowerOff size={10}/> INACTIVE</>}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-[3rem] border-2 border-dashed border-gray-200 p-24 flex flex-col items-center justify-center text-center shadow-sm max-w-4xl mx-auto">
          <div className="w-28 h-28 bg-gray-50 rounded-[2.5rem] flex items-center justify-center mb-8 border border-gray-100">
            <FaStethoscope className="text-5xl text-gray-200" />
          </div>
          <h3 className="text-2xl font-black text-gray-800 mb-2 tracking-tight">No Active Doctor Promos</h3>
          <p className="text-sm text-gray-400 max-w-xs mx-auto leading-relaxed font-medium">
            Boost your appointment bookings by launching your first discount campaign!
          </p>
        </div>
      )}
      </div>
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-[450px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <div className="flex justify-between items-center p-8 border-b border-gray-50 bg-[#08B36A] text-white">
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest mt-1 opacity-70">Coupon Configuration</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="space-y-1">
                <label className="label-style">Coupon Name (Code)</label>
                <div className="relative">
                   ...
                   <input
                    ...
                    placeholder="E.G. DOCTOR20" required
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="label-style">Usage Limit</label>
                  <div className="relative">
                    <FaLayerGroup className="absolute right-5 top-5 text-gray-400 text-sm ml-4"/>
                    <input name="maxUsagePerUser" type="number" value={formData.maxUsagePerUser} onChange={handleChange} placeholder="1" required className="input-style pl-11" />
                  </div>
                </div>
              </div>
              ...
              >
                {loading ? <FaSyncAlt className="animate-spin"/> : (editingId ? 'Save Changes' : 'Launch Coupon')}
              </button>
            </form>
          </div>
        </div>
