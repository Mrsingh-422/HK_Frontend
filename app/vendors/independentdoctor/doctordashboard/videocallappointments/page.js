...
function Page() {
    ...
    const getDoctorIdFromToken = () => {
        ...
        try {
            ...
            return decoded.id || decoded._id || decoded.doctorId;
        } catch (e) {
    ...
    // Retrieve Doctor ID from cached user/doctor session profiles
    const getDoctorIdFromLocalStorage = () => {
        ...
        try {
            ...
            console.error("Failed to parse local doctor storage:", e);
        }
        return null;
    };

    // Resolve active doctor ID on initialization
    useEffect(() => {
        const resolveDoctorId = async () => {
            // 1. Try reading from cached configurations
            let id = getDoctorIdFromLocalStorage() || getDoctorIdFromToken();
            if (id) {
                setActiveDoctorId(id);
                return;
            }

            // 2. Fallback: Query profile endpoints to obtain correct MongoDB ID
            try {
                if (DoctorAPI.getDoctorProfile) {
                ...
                console.error("Unable to resolve dynamic doctor ID via endpoints:", err);
            }
        };
    ...
    // 3. Initialize socket.io connection on mount
    useEffect(() => {
    ...
    // 4. Room Join and Leave handling matching your server's schema
    useEffect(() => {
