...
export default function ManageBankingPage() {
  ...
  // 1. Fetch current bank details from the Wallet Stats API instead of Profile
  const fetchBankDetails = async () => {
    setFetching(true);
    try {
      const response = await LabVendorAPI.getWalletStats();
      ...
      console.error("Error loading bank settlement parameters", error);
  ...
  // 2. Submit bank detail changes using the specialized patch endpoint
  const handleSubmit = async (e) => {
    ...
    const bankPayload = {
      ...
      upiId: bankDetails.upiId || undefined // Keeps empty optional values clean in DB
    };
    ...
    try {
      ...
      if (res.success) {
        ...
        await fetchBankDetails(); // Reload dynamically from wallet stats to get clean verified status
      } else {
  ...
  return (
    <div className="w-full mx-auto pb-20 px-4 md:px-0">
      ...
      <div className="mb-12 flex flex-col items-center text-center">
        ...
        <p className="text-sm text-gray-500 mt-2 font-semibold font-sans">Configure your bank account details for direct wallet payouts</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 md:col-span-2 space-y-6">
          ...
          <div className="flex justify-end pt-4 border-t border-gray-50">
            <button 
              type="submit" 
              disabled={saving} 
              className="px-8 py-4 bg-[#08B36A] hover:bg-emerald-600 text-white font-black rounded-2xl shadow-lg shadow-green-500/10 transition flex items-center gap-2 active:scale-95 text-xs uppercase tracking-wider font-sans"
            >
