...
export default function PromotionsPage() {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false); // Controls the Create/Edit Modal
  ...
  const fetchCoupons = async () => {
    try {
      setLoading(true);
      const res = await LabVendorAPI.listCoupons();
      if (res.success) {
        setCoupons(res.data || []);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await LabVendorAPI.updateCoupon(editingId, payload);
        toast.success('Coupon Updated Successfully!');
      } else {
        await LabVendorAPI.addCoupon(payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this coupon?")) {
      try {
        await LabVendorAPI.deleteCoupon(id);
  ...
  const handleToggleStatus = async (id) => {
    try {
      const res = await LabVendorAPI.toggleCoupon(id);
    ...
    <div className="w-full pb-20 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium text-sm mt-2">Manage and monitor your promotional discount codes.</p>
        </div>
      ...
      {coupons.length > 0 ? (
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {coupon.isAdminCreated && (
                      <span className="bg-indigo-100 text-indigo-700 text-[8px] font-black px-2 py-0.5 rounded-md border border-indigo-200">ADMIN GLOBAL</span>
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-50">
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-2 text-[9px] font-black px-4 py-2 rounded-xl border transition-all active:scale-95 ${coupon.isActive ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-600 border-red-200'}`}
                  >
        ...
        <div className="bg-white rounded-[3rem] border-2 border-dashed border-gray-200 p-24 flex flex-col items-center justify-center text-center shadow-sm max-w-4xl mx-auto">
          ...
          <p className="text-sm text-gray-400 max-w-xs mx-auto leading-relaxed font-medium">
            Your inventory is currently empty. Click the button above to launch your first marketing campaign!
          </p>
        </div>
      )}
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-[450px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <div className="flex justify-between items-center p-8 border-b border-gray-50 bg-[#08B36A] text-white">
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest mt-1 opacity-70">Global Configuration</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                      <FaTag    className="absolute   top-5 text-gray-400 text-sm"/>
                   <input
                    name="couponName" type="text" value={formData.couponName} onChange={handleChange}
                    placeholder="E.G. NEWYEAR2026" required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <FaPercent className="absolute  top-5 text-[#08B36A] text-xs"/>
                    <input name="discountPercentage" type="number" value={formData.discountPercentage} onChange={handleChange} placeholder="0" required className="input-style pl-10" />
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="label-style">Max Cap (₹)</label>
                  <div className="relative">
                    <FaRupeeSign className="absolute  top-5 text-[#08B36A] text-xs"/>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <FaLayerGroup className="absolute  top-5 text-gray-400 text-sm"/>
