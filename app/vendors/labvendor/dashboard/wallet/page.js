...
export default function LabWalletPage() {
  ...
  const loadWalletData = useCallback(async () => {
    ...
    try {
      ...
      if (statsRes?.success) {
        ...
        setFormError(statsRes?.message || 'Failed to load Lab wallet stats.');
      }
  ...
  const handleWithdraw = async (e) => {
    ...
    if (!bank?.isVerified) {
      setFormError('Payouts locked: Lab bank account is awaiting Admin verification.');
      return;
    }
  ...
  const handleBankSubmit = async (e) => {
    ...
    try {
      setBankLoading(true);
      const res = await LabVendorAPI.updateBankDetails(bankForm);

      if (res?.success) {
        setModalMsg({
          type: 'success',
          text: res.message || 'Bank details updated successfully. Payouts are locked until Admin verifies your account.'
        });
        setTimeout(async () => {
    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-10 min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1e3a8a] flex items-center gap-2">
            <FaFlask className="text-[#08B36A]" /> Lab Diagnostic Financial Wallet
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Diagnostic test gross billings, platform commission, 7-day cleared funds, and bank settlements.
          </p>
        </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Gross Lab Earnings</span>
          <p className="text-2xl font-black text-slate-800 mt-1">₹{(stats?.grossEarnings || 0).toLocaleString('en-IN')}</p>
          <p className="text-[10px] text-slate-400 mt-1">Total lab test sales</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
          ...
          <div className="border-t border-slate-100 pt-4 mt-4">
            <p className="text-[11px] text-slate-400 font-semibold italic">
              Unlocks automatically after 7-day post-completion safety lock window.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Lab Account Status</span>
              <span className="bg-slate-100 text-slate-700 text-[10px] px-2 py-0.5 rounded font-bold">Lab Provider</span>
            </div>
            <div className="mt-3">
              {bank?.isVerified ? (
          ...
          <div className="border-t border-slate-100 pt-4 mt-4 text-xs font-semibold text-slate-400">
            Payout unlocked upon Admin verification
          </div>
        </div>
      </section>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl animate-in zoom-in duration-150">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <FaUniversity className="text-[#08B36A]" /> Update Lab Bank Settlement
              </h3>
