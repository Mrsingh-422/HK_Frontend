...
const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Lab" },
...
export default function LabIssuesPage() {
  ...
  const labCategorySuggestions = [
    "Sample Collection Delay",
    "Test Report PDF Sync",
    "Bar-code Scanner Glitch",
    "Lab Payout & Wallet",
    "Phlebotomist Assignment",
    "Patient Booking Error",
    "Equipment Diagnostic Lag",
    "App & Technical Bug",
  ];
  ...
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await LabVendorAPI.getMyIssues(params);
      ...
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Lab authentication session expired or missing token. Please log in again.", "error");
      } else {
        showAlert(err.response?.data?.message || "Failed to load lab tickets", "error");
      }
    } finally {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!formData.title.trim() || !formData.detailedDescription.trim()) {
      showAlert("Please enter both headline title and detailed description.", "error");
      return;
    }
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Diagnostic Support");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Lab Vendor Web / Next.js");

      files.forEach((file) => {
        data.append("attachments", file);
      });

      const res = await LabVendorAPI.createIssue(data);
      const resData = res?.data || res;
      if (resData?.success) {
        showAlert("Issue ticket submitted successfully to Technical Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
      ...
      showAlert(err.response?.data?.message || "Failed to submit ticket. Please check lab login session.", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await LabVendorAPI.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await LabVendorAPI.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await LabVendorAPI.trackIssue(issueItem._id);
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "RESOLVED":
        return <span className="px-2.5 py-1 rounded-md text-[10px] font-black uppercase bg-teal-50 text-teal-700 border border-teal-200">RESOLVED</span>;
      case "IN PROGRESS":
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {alertBanner && (
          <div className={`p-4 rounded-2xl flex items-center justify-between text-xs font-bold shadow-lg animate-in fade-in duration-200 ${
            alertBanner.type === "error" ? "bg-rose-50 border border-rose-200 text-rose-800" : "bg-teal-50 border border-teal-200 text-teal-800"
          }`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <FlaskConical className="text-teal-600" size={28} /> Lab Helpdesk & Incident Hub
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Submit diagnostic anomalies, track barcode/report sync bugs, and monitor support resolutions.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "list" ? "bg-teal-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <ClipboardList size={14} /> My Lab Tickets
            </button>
            <button
              onClick={() => setActiveTab("create")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "create" ? "bg-teal-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <Plus size={14} /> Log New Issue
            </button>
            <button
              onClick={() => setActiveTab("track")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "track" ? "bg-teal-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for your lab session. Please re-login to the lab dashboard.</span>
            </div>
            <button
              onClick={fetchIssues}
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search in headline, category, or Ticket ID..."
                  value={search}
                  onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }}
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-teal-500"
                />
              </div>
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching lab incident registry...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                <AlertTriangle className="mx-auto text-slate-300 w-12 h-12" />
                <h3 className="text-base font-bold text-slate-700">No Lab Incident Records Found</h3>
                <p className="text-xs text-slate-400">No active tickets reported under your lab provider account.</p>
                <button
                  onClick={() => setActiveTab("create")}
                  className="bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow"
                >
                  Log An Incident Now
                </button>
              </div>
                  ...
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-[11px] font-black text-teal-700 bg-teal-50 px-2 py-0.5 rounded border border-teal-100">
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
                <span>Total Lab Tickets: {totalRecords} | Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <FlaskConical className="text-teal-600" size={20} /> Report Lab Operational or Technical Glitch
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Our 24/7 technical team will investigate barcode, reporting, or sample pipeline errors.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. CBC test report PDF fails to sync with Health Locker"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Type custom category (e.g. Phlebotomist OTP Lag, Barcode Printer Mismatch)..."
                value={formData.category}
                onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 mb-2"
              />
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Lab Suggestions:
                </span>
                {labCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => setFormData({ ...formData, category: cat })}
                    className="px-2 py-0.5 bg-slate-100 hover:bg-teal-50 hover:text-teal-700 text-slate-600 rounded-md text-[10px] font-bold transition"
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-700 outline-none cursor-pointer focus:border-teal-500"
              >
                <option value="Low">Low</option>
                <option value="Medium">Medium</option>
                <option value="High">High</option>
                <option value="Urgent">Urgent (Sample processing blocked)</option>
              </select>
            </div>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include order numbers, sample bar codes, or exact error prompt..."
                value={formData.detailedDescription}
                onChange={(e) => setFormData({ ...formData, detailedDescription: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-700 outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
              />
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">
                Attach Diagnostic Logs or Screenshots (Max 5)
              </label>
              <div className="border-2 border-dashed border-slate-200 hover:border-teal-500 rounded-2xl p-5 text-center cursor-pointer bg-slate-50/50 transition relative">
                <input
                  type="file"
                  multiple
                  accept="image/*,.pdf"
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-teal-600 hover:bg-teal-700 text-white px-8 py-3 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-2 disabled:opacity-50"
              >
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-teal-500"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loadingTrack}
                  className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3 rounded-2xl text-xs md:text-sm font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
            ...
            {trackingData && (
              <div className="space-y-6 animate-in zoom-in-95 duration-200">
                ...
                {trackingData.resolutionDetails?.resolutionNote && (
                  <div className="bg-teal-50 border border-teal-200 p-6 rounded-3xl space-y-2">
                    ...
                    {trackingData.resolutionDetails.resolvedAt && (
                      <span className="text-[11px] text-teal-700 font-mono block pt-1">
                ...
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-6">
                  <h3 className="font-black text-base text-slate-900 tracking-tight flex items-center gap-2">
                    <Clock size={18} className="text-teal-600" /> Resolution Audit Timeline
                  </h3>

                  {trackingData.timeline && trackingData.timeline.length > 0 ? (
                    <div className="relative border-l-2 border-teal-500/30 ml-4 pl-6 space-y-6">
                      {trackingData.timeline.map((entry, idx) => (
                        <div key={idx} className="relative space-y-1">
                          <div className="w-3.5 h-3.5 bg-teal-600 ring-4 ring-teal-100 rounded-full absolute -left-[31px] top-1" />
                          <div className="flex flex-wrap items-center gap-2">
