...
export default function AvailabilityPage() {
  ...
  const fetchAvailability = async () => {
    try {
      setLoading(true);
      const res = await DoctorAPI.getMySlots();
      
      // Access config logic matching reference
      ...
      if (data) {
        setAvailability({
          ...
          maxClientsPerSlot: data.maxClientsPerSlot || 1,
  ...
  const generateSlotsLocally = () => {
    ...
    const endT = availability.endTime || '18:00';
  ...
  const removeBlockedDate = (dateToRemove) => {
    ...
    toast.success("Date removed from list");
  };
  ...
  const toggleBlockSlot = async (slotTime, isCurrentlyBlocked) => {
    try {
      setLoading(true);
      // Logic from reference: update array and save whole payload
      ...
      toast.success(isCurrentlyBlocked ? `Slot ${slotTime} Unblocked` : `Slot ${slotTime} Blocked`);
    } catch (error) {
  ...
  const getCombinedSlots = () => {
    ...
    // Check if today is a blocked date (Reference logic)
    ...
    <div className="w-full relative pb-10">
          ...
          <div className="border-t border-gray-100 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 items-end bg-red-50/30 p-5 rounded-xl border border-red-100">
                <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-gray-500 mb-1">Pick a Date to Close Practice</label>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        {filteredSlots.length === 0 && (
           <div className="p-10 text-center text-gray-500 italic">No slots available. Refresh to try again.</div>
        )}
      </div>
