...
const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Pharmacy" },
...
export default function PharmacyIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "High",
    platform: "Web",
    appVersion: "Pharmacy Orders & Inventory Portal",
  });
  ...
  const pharmacyCategorySuggestions = [
    "Prescription (Rx) Order Sync Error",
    "Medicine Inventory & Stock Mismatch",
    "Delivery Rider Dispatch Delay",
    "Pharmacy Payout & Settlement Lag",
    "Order Cancellation & Refund Glitch",
    "Substitute Medicine Approval Delay",
    "Barcode / Batch Expiry Scan Bug",
    "App & Technical Bug",
  ];
  ...
  // 3.2 FETCH PHARMACY ISSUES LIST
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await PharmacyVendorAPI.getMyIssues(params);
      ...
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Pharmacy session expired or missing token. Please log in again.", "error");
      } else {
        showAlert(err.response?.data?.message || "Failed to load pharmacy tickets", "error");
      }
    } finally {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Pharmacy Orders & Delivery");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Pharmacy Web Portal / Next.js");

      files.forEach((file) => {
        data.append("attachments", file);
      });

      const res = await PharmacyVendorAPI.createIssue(data);
      const resData = res?.data || res;
      if (resData?.success) {
        showAlert("Pharmacy issue ticket logged successfully to 24/7 Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
      ...
      showAlert(err.response?.data?.message || "Failed to submit ticket. Please check pharmacy login session.", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await PharmacyVendorAPI.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await PharmacyVendorAPI.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await PharmacyVendorAPI.trackIssue(issueItem._id);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 font-sans text-slate-800">
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <Pill className="text-emerald-600" size={28} /> Pharmacy Incident & Order Support Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Report Rx order sync delays, medicine inventory discrepancies, and rider dispatch issues.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "list" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <ClipboardList size={14} /> My Pharmacy Tickets
            </button>
            <button
              onClick={() => setActiveTab("create")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "create" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <Plus size={14} /> Log Pharmacy Issue
            </button>
            <button
              onClick={() => setActiveTab("track")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "track" ? "bg-emerald-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for your pharmacy session. Please re-login.</span>
            </div>
            <button
              onClick={fetchIssues}
        ...
        {/* ========================================================================= */}
        {/* TAB 1: MY PHARMACY TICKETS LIST */}
        {/* ========================================================================= */}
        {activeTab === "list" && (
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-emerald-500"
                />
              </div>
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching pharmacy incident records...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                <AlertTriangle className="mx-auto text-slate-300 w-12 h-12" />
                <h3 className="text-base font-bold text-slate-700">No Pharmacy Tickets Found</h3>
                <p className="text-xs text-slate-400">No active incidents reported under your pharmacy account.</p>
                <button
                  onClick={() => setActiveTab("create")}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow"
                >
                  Log A Pharmacy Anomaly
                </button>
              </div>
            ) : (
                  ...
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-[11px] font-black text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
                <span>Total Pharmacy Tickets: {totalRecords} | Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <Pill className="text-emerald-600" size={20} /> Report Pharmacy Order or Inventory Glitch
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit prescription sync issues, rider dispatch delays, or inventory discrepancies.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Prescription PDF verification fails on digital Rx orders"
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Type custom category (e.g. Delivery Rider Timeout, Batch Expiry Scanner)..."
              ...
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Pharmacy Presets:
                </span>
                {pharmacyCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
            ...
            <div>
              ...
              >
                <option value="Urgent">Urgent (Express medicine delivery blocked)</option>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include order numbers, medicine names, rider contact details, or exact error prompts..."
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-3 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-2 disabled:opacity-50"
              >
                {submitting ? <Loader2 size={16} className="animate-spin" /> : null}
                Submit Pharmacy Issue
              </button>
            </div>
          </form>
        )}
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-emerald-500"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loadingTrack}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-2xl text-xs md:text-sm font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
              ...
              <div className="space-y-6 animate-in zoom-in-95 duration-200">
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-100">
                ...
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-6">
                  <h3 className="font-black text-base text-slate-900 tracking-tight flex items-center gap-2">
                    <Clock size={18} className="text-emerald-600" /> Resolution Audit Timeline
                  </h3>
                    ...
                    <div className="relative border-l-2 border-emerald-500/30 ml-4 pl-6 space-y-6">
                      {trackingData.timeline.map((entry, idx) => (
                        <div key={idx} className="relative space-y-1">
                          <div className="w-3.5 h-3.5 bg-emerald-600 ring-4 ring-emerald-100 rounded-full absolute -left-[31px] top-1" />
                          <div className="flex flex-wrap items-center gap-2">
