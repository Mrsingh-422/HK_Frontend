...
export default function ManageBankingPage() {
  ...
  // 1. Fetch bank details directly from the Wallet Stats API
  const fetchBankDetails = async () => {
    setFetching(true);
    try {
      const response = await PharmacyVendorAPI.getWalletStats();
      // Using response.bankDetails based on the GET /provider/wallet/stats payload layout
      ...
      console.error("Error fetching pharmacy bank settings", error);
  ...
  const handleSubmit = async (e) => {
    ...
    const bankPayload = {
      ...
      upiId: bankDetails.upiId || undefined // Keeps empty string values clean in DB
    };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 font-sans">
            ...
            <div className="sm:col-span-2 space-y-1">
              ...
              <input 
                ...
                onChange={handleInputChange} 
                className="input-style font-mono"
                placeholder="e.g. pharmacyaccount@okaxis"
              />
            </div>
          </div>
