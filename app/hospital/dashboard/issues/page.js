...
const LIFECYCLE_STEPS = [
  { key: "OPEN", label: "Ticket Logged", desc: "Submitted by Hospital" },
...
export default function HospitalIssuesPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    priority: "High",
    platform: "Web",
    appVersion: "Hospital Management Panel",
  });
  ...
  const hospitalCategorySuggestions = [
    "ICU Bed Allocation Conflict",
    "Emergency Bed Sync",
    "OT Slot Booking Glitch",
    "Doctor On-Call Availability",
    "Patient Discharge & Billing",
    "Hospital Bed Status Not Updating",
    "Ambulance Transfer SOS Delay",
    "Hospital Wallet & Payout",
  ];
  ...
  // 3.2 FETCH HOSPITAL ISSUES LIST
  const fetchIssues = useCallback(async () => {
    ...
    try {
      ...
      const res = await HospitalAPI.getMyIssues(params);
      ...
      if (err.response?.status === 401) {
        setAuthError(true);
        showAlert("Hospital session expired or missing token. Please re-login.", "error");
      } else {
        showAlert(err.response?.data?.message || "Failed to load hospital tickets", "error");
      }
    } finally {
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!formData.title.trim() || !formData.detailedDescription.trim()) {
      showAlert("Please enter both issue title and detailed description.", "error");
      return;
    }
    ...
    try {
      ...
      data.append("category", formData.category.trim() || "Hospital Bed Booking");
      data.append("priority", formData.priority);
      data.append("platform", "Web");
      data.append("appVersion", "Hospital Web Panel / Next.js");

      files.forEach((file) => {
        data.append("attachments", file);
      });

      const res = await HospitalAPI.createIssue(data);
      const resData = res?.data || res;
      if (resData?.success) {
        showAlert("Hospital issue ticket logged successfully to 24/7 Support!", "success");
        setFormData({
          title: "",
          detailedDescription: "",
          category: "",
      ...
      showAlert(err.response?.data?.message || "Failed to submit ticket. Please check hospital login session.", "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleTrackSearch = async (idToSearch) => {
    ...
    try {
      if (target.length === 24) {
        const res = await HospitalAPI.trackIssue(target);
        const resData = res?.data || res;
      ...
      const listRes = await HospitalAPI.getMyIssues({ search: target, limit: 1 });
      const listData = listRes?.data || listRes;
      if (listData?.data && listData.data.length > 0) {
        const issueItem = listData.data[0];
        const trackRes = await HospitalAPI.trackIssue(issueItem._id);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {alertBanner && (
          <div className={`p-4 rounded-2xl flex items-center justify-between text-xs font-bold shadow-lg animate-in fade-in duration-200 ${
            alertBanner.type === "error" ? "bg-rose-50 border border-rose-200 text-rose-800" : "bg-blue-50 border border-blue-200 text-blue-800"
          }`}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <Building2 className="text-blue-600" size={28} /> Hospital Support & Incident Desk
              </h1>
              <p className="text-xs text-slate-500 mt-0.5 font-medium">
                Log bed booking anomalies, OT synchronization issues, and track live support progress.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1.5">
            ...
            >
              <ClipboardList size={14} /> My Hospital Tickets
            </button>
            <button
              onClick={() => setActiveTab("create")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "create" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
              <Plus size={14} /> Log Hospital Issue
            </button>
            <button
              onClick={() => setActiveTab("track")}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === "track" ? "bg-blue-600 text-white shadow-sm" : "text-slate-600 hover:bg-slate-50"
              }`}
            >
        ...
        {authError && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3 text-xs text-amber-900 font-bold">
            ...
            <div className="flex-1">
              <span>Authentication token is missing or expired for your hospital session. Please re-login to the hospital panel.</span>
            </div>
            <button
              onClick={fetchIssues}
        ...
        {/* ========================================================================= */}
        {/* TAB 1: MY HOSPITAL TICKETS LIST */}
        {/* ========================================================================= */}
        {activeTab === "list" && (
          <div className="space-y-4 animate-in fade-in duration-200">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-blue-500"
                />
              </div>
            ...
            {loadingList ? (
              <div className="text-center py-20 text-slate-400">
                ...
                <span className="text-xs font-bold">Fetching hospital incident registry...</span>
              </div>
            ) : issues.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-3">
                <AlertTriangle className="mx-auto text-slate-300 w-12 h-12" />
                <h3 className="text-base font-bold text-slate-700">No Hospital Incident Records Found</h3>
                <p className="text-xs text-slate-400">No active tickets reported under your hospital provider account.</p>
                <button
                  onClick={() => setActiveTab("create")}
                  className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow"
                >
                  Log An Incident Now
                </button>
              </div>
            ...
            {totalPages > 1 && (
              <div className="p-4 bg-white rounded-2xl border border-slate-200 flex items-center justify-between text-xs font-bold text-slate-500">
                <span>Total Hospital Tickets: {totalRecords} | Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button
          ...
          >
            <div>
              <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                <Building2 className="text-blue-600" size={20} /> Report Hospital Bed / Operation Anomaly
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                Submit bed sync glitches, doctor scheduling conflicts, or emergency admission latency.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. ICU Bed #402 allocation fails on emergency transfer"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Type custom category (e.g. OT Slot Conflict, Bed Booking OTP Delay)..."
                value={formData.category}
                onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 mb-2"
              />
              <div className="flex flex-wrap gap-1.5 items-center">
                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                  <Sparkles size={10} /> Hospital Presets:
                </span>
                {hospitalCategorySuggestions.map((cat) => (
                  <button
                    type="button"
                    key={cat}
                    onClick={() => setFormData({ ...formData, category: cat })}
                    className="px-2 py-0.5 bg-slate-100 hover:bg-blue-50 hover:text-blue-700 text-slate-600 rounded-md text-[10px] font-bold transition"
                  >
                    {cat}
                  </button>
                ))}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-700 outline-none cursor-pointer focus:border-blue-500"
              >
                <option value="Urgent">Urgent (Emergency admissions blocked)</option>
                <option value="High">High (Bed sync latency)</option>
            ...
            <div>
              ...
              <textarea
                rows={4}
                required
                placeholder="Include patient IDs, bed numbers, ward units, or exact system error prompts..."
                value={formData.detailedDescription}
                onChange={(e) => setFormData({ ...formData, detailedDescription: e.target.value })}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1.5">
                Attach Screenshots or System Logs (Max 5)
              </label>
              <div className="border-2 border-dashed border-slate-200 hover:border-blue-500 rounded-2xl p-5 text-center cursor-pointer bg-slate-50/50 transition relative">
                <input
                  type="file"
                  multiple
                  accept="image/*,.pdf"
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-2 disabled:opacity-50"
              >
                {submitting ? <Loader2 size={16} className="animate-spin" /> : null}
                Submit Hospital Issue
              </button>
            </div>
          </form>
        )}
              ...
              >
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs md:text-sm font-bold text-slate-800 outline-none focus:border-blue-500"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loadingTrack}
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl text-xs md:text-sm font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
              ...
              <div className="space-y-6 animate-in zoom-in-95 duration-200">
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-black text-blue-700 bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-100">
                ...
                {trackingData.resolutionDetails?.resolutionNote && (
                  <div className="bg-blue-50 border border-blue-200 p-6 rounded-3xl space-y-2">
                ...
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-md space-y-6">
                  <h3 className="font-black text-base text-slate-900 tracking-tight flex items-center gap-2">
                    <Clock size={18} className="text-blue-600" /> Resolution Audit Timeline
                  </h3>
                    ...
                    <div className="relative border-l-2 border-blue-500/30 ml-4 pl-6 space-y-6">
                      {trackingData.timeline.map((entry, idx) => (
                        <div key={idx} className="relative space-y-1">
                          <div className="w-3.5 h-3.5 bg-blue-600 ring-4 ring-blue-100 rounded-full absolute -left-[31px] top-1" />
                          <div className="flex flex-wrap items-center gap-2">
