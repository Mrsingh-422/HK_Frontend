...
const CompleteDischargeModal = ({ patient, onClose, onConfirm }) => {
  const [billingItems, setBillingItems] = useState([
    { serviceName: "Laboratory Blood Tests", price: 1200 },
    { serviceName: "Pharmacy Medicines charges", price: 1800 }
  ]);
  ...
  const totalCalculatedCost = billingItems.reduce((sum, item) => sum + item.price, 0);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (billingItems.length === 0) {
      alert("Please add at least one line item to finalize calculations.");
      return;
    }
    ...
    onConfirm({
      ...
      isEmergency: !!patient.ambulanceId // Check if dynamic emergency ambulance release applies
    });
        ...
        <div className="p-8 md:p-10 max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-150 space-y-3">
              <span className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Add Treatment Charge</span>
              <div className="grid grid-cols-12 gap-2">
                ...
                <button 
                  ...
                  className="col-span-2 bg-[#08B36A] hover:bg-[#079d5c] text-white rounded-xl flex items-center justify-center transition shadow-sm shadow-[#08B36A]/10"
                >
                  <FaPlus size={12} />
                </button>
              </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-sm font-black text-slate-900">
              <span className="uppercase tracking-wide text-xs text-slate-400">Total Bill Summary</span>
