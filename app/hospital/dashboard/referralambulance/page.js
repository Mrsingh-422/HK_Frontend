          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr><td colSpan="7" className="py-20 text-center font-bold text-gray-400">No referral bookings found.</td></tr>
              ) : bookings.map((item) => (
                <tr key={item._id} className="hover:bg-[#08B36A]/5 transition-colors group cursor-pointer" onClick={() => setSelectedBooking(item)}>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                        ...
                        <span className={`text-[10px] font-black uppercase ${item.triageLevel === 'Urgent' ? 'text-red-500' : 'text-[#08B36A]'}`}>{item.triageLevel} • {item.patientDetails?.condition}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-[#08B36A] font-bold uppercase">{item.ambulanceId?.vehicleType}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                     <div className="flex flex-col">
                        <span className="text-xs font-bold text-gray-700">{formatDate(item.scheduledAt)}</span>
                        <span className="text-[10px] text-gray-400 font-bold">{item.scheduledTime || 'Immediate'}</span>
                     </div>
                  </td>
                  <td className="px-6 py-5">
                    <span className="bg-[#08B36A]/10 text-[#08B36A] text-[10px] font-black px-3 py-1 rounded-full border border-[#08B36A]/20">
                        {item.status}
                    </span>
                  </td>
